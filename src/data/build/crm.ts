    ...
    // 6 · Danh bạ doanh nghiệp (free company data) ────────────────────────────
    {
      ...
      detail: {
          ...
          },
          {
            ...
            table: {
              ...
              rows: [
                ...
                ['Stat strip', 'Hạng · Customer since · Open jobs · Team · Job quota · CV unlocks · Sales owner', 'Nguồn · Ngày nhập · Người liên hệ · SĐT · **Yêu cầu** (count + who asked first). Six dashes in a row is not a dashboard.'],
                ...
                ['Owner history tab', 'Owner tenure chain; the claim chain only while the company is still IN the pool', '**On the pool record**, Lịch sử yêu cầu nhận renders in full — there it is decision context, not a history display: the admin assigns on this page, and a re-submitted weak request must be visible next to the fresh one (it is what “đã từ chối N lần” points to; a pool row also has no other ownership story). **On a promoted CRM record it does NOT render** — Yêu cầu nhận công ty owns request history, and repeating the list on every promoted record would be the same data maintained twice. What stays is the one line that IS ownership history: the first tenure entry, “Nhận từ Free data — duyệt bởi {admin}”, plus a pointer to the tracking page. A rejected request in a WON contest reads “công ty được phân cho sales khác”, not “về lại Chưa nhận”. Worked examples: **CO-XHWCYJB** (5 handovers, no claims) and **CO-N3V7F5Z** (first tenure from the claim → one rebalance handover → pointer to the tracking page).'],
        ...
        rules: [
          ...
          'A row already Đang chờ duyệt DOES accept further requests. The count is shown on the pool row, in the claim form and on the record; approving one turns the others to Từ chối.',
        ...
        states: ['— pool row —', 'Chưa nhận', 'Đang chờ duyệt', 'Đã nhận (leaves the list)', '— request —', 'Đang chờ', 'Đã duyệt', 'Từ chối'],
        backend: {
          dataModel: [
          ...
          endpoints: [
            ...
            'POST /admin/crm/company-claims/:id/approve — creates the company, sets salesOwner = requestedBy, creates contact #1 from the contact point, sets pool state = claimed + claimedCompanyId (which removes it from the list), and sets every competing claim on that company to rejected',
            'POST /admin/crm/company-claims/:id/reject — no body; pool state returns to free if no other claim is pending. There is no reason field.',
