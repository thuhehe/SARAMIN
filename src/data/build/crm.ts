      ...
      detail: {
        ...
        },
        {
          ...
          table: {
            ...
            rows: [
                        ['Thông tin cơ bản', 'Tên hiển thị, industry, size, tags, quốc tịch, tỉnh/thành, website', 'Tên hiển thị'],
    ...
    // 2 · Quotations ──────────────────────────────────────────────────
    {
      ...
      detail: {
        ...
        states: [
          ...
          {
            label: 'Chiết khấu đặc biệt — the one number a rep may type, and who signs it',
            text: 'A quotation is discounted in exactly one of two ways, and they are **mutually exclusive**.\n\nBy default the **programme** applies: it follows from the customer’s status, lands per line, is read-only and needs nobody’s approval. Ticking **Chiết khấu đặc biệt** turns the programme off for that quotation and lets the rep type **one** percentage against each option’s subtotal — the “chiết khấu tổng hóa đơn”. That number then has to be approved before the quotation can go to the customer.',
            table: {
              cols: ['', 'Programme (default)', 'Chiết khấu đặc biệt'],
              rows: [
                ['Chosen by', 'The customer’s status — nobody picks it', 'The rep, deliberately, with a checkbox'],
                ['Where the discount lands', 'Per line, from that line’s own quantity', 'One figure on the **option subtotal**, before VAT'],
                ['Can a rep edit the line %?', '**No**', '**No** — the line cells stay 0 and stay locked'],
                ['Approval', 'None. The rate was approved when the programme was configured', '**Required** — see the bands below'],
                ['The other one', '—', 'The programme is **not** applied on top. Never both.'],
              ],
            },
            items: [
              'Per-line editing is impossible in **both** modes, and that is the point of the feature rather than a limitation of it. A concession spread across five lines cannot be judged at a glance, and an approver would be signing off on an arithmetic exercise instead of on one number.',
              'The two never stack. A special rate layered on a programme rate produces a total nobody planned and an approver who has signed off on half of it.',
              'Ticking the box **zeroes** the option figure rather than pre-filling it with what the programme was giving. The rep starts from nothing, so “what did we actually decide to give away” has an answer.',
              'The screen states what the customer is losing: with the box on, the banner names the programme that has stopped applying. A rep who types 8% for a customer entitled to 30% should be able to see that they have made the offer worse.',
            ],
          },
          {
            label: 'Special-discount approval — the bands, and the flow',
            text: 'Approval is a gate on the **quotation**, not on the PO: by the time an order exists the price is already committed. It routes on the **highest** rate in the document — options are alternatives and the customer picks one, so the approver has to be able to sign off on the worst case rather than on an average.',
            table: {
              cols: ['Chiết khấu tổng hóa đơn', 'Approver', 'Note'],
              rows: [
                ['0%', '— none', 'Nothing to approve. The quotation exports normally.'],
                ['1 – 10%', '**Sales leader**', 'The rep’s own team lead.'],
                ['> 10%', '**Sales manager**', 'Goes **straight** to the manager. It does not queue behind the leader first.'],
              ],
            },
            items: [
              'Escalation is by **amount, not by chain**. Two signatures on one discount slows the deal without adding a second real decision, so above 10% the leader is skipped entirely. If the client wants both, it is a one-line change — but it roughly doubles the time a large discount sits waiting.',
              'The flow: rep ticks the box and types the % → **Gửi {approver} duyệt** → the quotation stays **Draft** and carries an approval request assigned to the right role → the approver opens it and either **Duyệt** or **Từ chối** with a reason → approved unlocks Send / Export; rejected returns it to the rep with the reason attached.',
              '**Changing the % after approval voids it.** The request re-routes from scratch, and to the higher band if the new figure crosses 10%. Without this the approval means nothing — a rep could get 5% signed and send 25%.',
              'The approval is recorded on the quotation: who approved, when, and **at what percentage**. That last field is what makes the void-on-change rule enforceable, and it is what an audit reads.',
              'A revised quotation (v2) needs its **own** approval. Approval attaches to the version, not to the deal.',
              'The button names the **person**, not the state — “Gửi Sales leader duyệt”, never “Request approval”. A rep who does not know who to chase is a quotation that sits for three days.',
              'Only the quotation is gated. Once approved and issued to a PO the price travels down the chain unchanged — the PO and the invoice have no discount control of their own.',
            ],
            warn: 'Open — what happens when the approver is the requester. A sales leader raising their own quotation cannot approve it, so it has to escalate to the manager; and a sales manager’s own special discount has nobody above them in this model. Confirm the rule before build.',
          },
          {
            label: 'Where approval happens — no new screen',
            text: 'The decision is taken on the **quotation itself**, and the approver finds their requests through a filter on the **existing Quotations list**. There is no “Approvals” page.\n\nApproving a percentage without the lines, the options and the customer in front of you is signing a number blind — so the action has to live where the document is. And a separate queue screen would be a second, thinner copy of the quotation list plus a link back to it; what an approver actually needs is a way to FIND their requests, which is a filter, not a page.',
            table: {
              ...
              rows: [
                ...
                ['**Sales lead**', 'Quotations list → **⏳ Chờ tôi duyệt (n)**', 'Sees only 1–10% requests **from their own team**. Opens the quotation → **Duyệt** or **Từ chối**.'],
            ...
            items: [
              ...
              'Who is the approver is answered by the SAME persona switcher the Companies list uses, over the same SALES_TEAMS org — one answer to “who am I”, everywhere. A lead approves their own team’s requests; there is one department manager.',
              'A rejection **requires a reason**; an approval does not. A rep can only act on “no” if they are told what would be a yes, so the field is mandatory and the placeholder asks for the number that would pass.',
              'The bar states the request in full — rate, requester, timestamp, the rep’s note — and after a decision it adds who decided, when, and **at what percentage**. That last field is what makes “editing the % voids the approval” enforceable.',
              'Notification is out of scope here but assumed: the approver gets one when a request routes to them, and the rep gets one on the decision. The shell already carries a bell.',
            ],
          },
          {
            ...
            table: {
              ...
              rows: [
                ...
                ['No programme', '—', 'Cannot happen: the three customer statuses cover every company, so a programme always matches. Kept in the model only so a future status has a defined behaviour.'],
              ],
            },
            items: [
              'The discount cells are **read-only, always**, and there is no toggle that hands them back. The number follows from the quantity and the customer status; it is not a negotiating position a rep sets on this screen.',
              ...
              'A programme rate needs **no approval** — it was approved when the programme was configured, and no rep can type a different one. Approval exists only for **Chiết khấu đặc biệt**, which is a separate, opt-in mode; see the two blocks above.',
        ...
        backend: {
          ...
          endpoints: [
            ...
            'POST /admin/crm/quotations/:id/request-approval — routes by pct: ≤10% sales_leader, >10% sales_manager',
    ...
    // 6 · Danh bạ doanh nghiệp (free company data) ────────────────────────────
    {
      ...
      detail: {
        requirements: [
          ...
          },
          {
            ...
            table: {
              ...
              rows: [
                ...
                ['Đang chờ duyệt', 'At least one rep has asked; admin has not decided. The row names the first requester and, when there is more than one, the request **count**.', '**Still Xin nhận** — the button reads “Xin nhận (đã có người xin)” and the form says how many asked and who was first. A second request is allowed; see the competing-requests rule.'],
          ...
          {
            label: 'Tạo yêu cầu — four inputs, every one of them used downstream',
            text: 'A rep cannot take a company by clicking. The request form asks for four things, and each is consumed by somebody later rather than merely collected — which is what stops it becoming a formality every rep passes.',
            table: {
              cols: ['Input', 'Required', 'What it is used for afterwards'],
              rows: [
                ['**Lý do tạo yêu cầu** (mô tả chi tiết)', 'YES', 'The first thing the approver reads. Free text — but see the note below on why a template in the placeholder is not enough.'],
                ['**Phân loại khách hàng trong Free Data**', 'YES', 'A fixed list of 8. This is what makes a queue of hundreds reviewable at a glance, and what lets HQ measure **which pool segment actually converts** instead of guessing which import batch to buy again.'],
                ['**Contact point** — tên người liên hệ (req) · SĐT (req) · email (opt)', 'YES', 'Becomes **contact #1** on the company the rep is about to receive. Structured fields, NOT a line typed inside the description — see below.'],
                ['**Bằng chứng đang tuyển** — link tin tuyển dụng OR tệp đính kèm', 'YES (one of the two)', 'A link the approver can **open**. A file is accepted but is second best: it has to be opened and believed. Neither is not an option — "đang tuyển" is one of the two conditions the request exists to prove.'],
              ],
            },
          },
          {
            label: 'Contact point is FIELDS, not a template in the placeholder',
            text: 'The current TopDev FreeDB form asks for the contact inside the description box, via a placeholder template (`1. Lý do:` / `2. Contact Point: Tên nhân sự – Email – SĐT`). The live queue shows what that produces: reasons reading **“test”**, **“đang tuyển”**, **“1 đang tuyển”** — and those rows are sitting in ĐANG CHỜ waiting for a human to approve them on no information at all.\n\nA placeholder is a **request**. A required field is not. So the contact point is three real inputs, and the reason box keeps only the reason.',
            table: {
              cols: ['Consequence', 'Behaviour'],
              rows: [
                ['The phone is structured', 'It can be written to contact #1 on approval. A phone number buried in free text cannot be — someone would have to re-type it, and nobody does.'],
                ['Pre-filled from the pool row', 'When the pool row already has a contact, the form starts filled and the rep corrects it. Cheaper than re-typing, and it surfaces stale source data.'],
                ['A one-word reason is visible as one', 'The queue renders a suspiciously short reason in **amber**, and a request with no evidence at all shows **⚠ không có bằng chứng**. The approver does not have to open a row to see it is empty.'],
              ],
            },
          },
          {
          ...
          },
          {
            ...
            table: {
              ...
              rows: [
                ...
                ['Decision rule', 'Approving ONE request auto-rejects the others on that company, with the note "đã duyệt cho sales khác" — the losing rep is told, and is told why, without the admin writing it twice. Recommended tie-break: **strength of evidence first, timestamp second** (a link beats a screenshot beats a note; earliest wins a tie).'],
              ],
            },
          },
          {
          ...
          },
          {
            ...
            table: {
              ...
              rows: [
                ...
                ['Từ chối', '**Ghi chú duyệt is required.** The pool row returns to **Chưa nhận**. The note is shown to the rep — a rep refused twice for the same reason should be able to read it. It also prints in the queue’s Ghi chú duyệt column, so the next approver sees the history.'],
                ['Approving one of several competing requests', 'The others on that company are auto-rejected with the note “đã duyệt cho sales khác”. One action, every rep informed.'],
                ['Resolved requests', '**Stay in the queue**, filterable by Trạng thái (Đang chờ · Được duyệt · Từ chối). The queue is the record of how each company entered the CRM; hiding resolved rows would leave the only copy of that in an audit log nobody opens.'],
          ...
          {
            label: 'Queue columns — what an approver needs without opening a row',
            text: 'The queue is reviewed in bulk, so every column exists to let a row be **decided or skipped without opening it**.',
            table: {
              cols: ['Column', 'Why it is there'],
              rows: [
                ['ID', 'The number a rep quotes when they ask why their request is still waiting.'],
                ['Công ty (danh bạ) + contact point', 'The company name with the submitted **người liên hệ / SĐT / email stacked underneath** — the approver is checking whether this rep actually has a way in, so the phone belongs next to the name, not three columns away.'],
                ['Sales xin · Ngày tạo yêu cầu', 'Who and when. Timestamp to the minute, because it is the tie-break of last resort.'],
                ['Lý do tạo yêu cầu + bằng chứng', 'One cell, because they are read together. A **link** renders as an openable link; a **file** as 📎 filename; nothing renders as **⚠ không có bằng chứng**. A suspiciously short reason renders amber.'],
                ['Phân loại khách hàng', 'The rep’s own classification — the fastest way to triage a long queue.'],
                ['Ghi chú duyệt', 'The decision note. Required on a rejection, optional on an approval.'],
                ['Số YC', 'Request count on that company. `2 ⚠` = a contest, not a routine approval.'],
                ['Trạng thái', 'Đang chờ · Được duyệt · Từ chối. Action buttons appear only on Đang chờ rows.'],
              ],
            },
          },
          {
        ...
        keyPoints: [
          ...
          { vi: 'Contact point phải là FIELD riêng, không phải template trong ô mô tả — hệ thống hiện tại dùng template và kết quả là lý do “test”, “đang tuyển” đang chờ duyệt.', en: 'Contact point must be real FIELDS, not a template in the description box — the current system uses a template and the queue is full of reasons reading "test".' },
        ...
        rules: [
          ...
          'Xin nhận requires four things: lý do, phân loại khách hàng (from the fixed list of 8), contact point (tên + SĐT required, email optional), and hiring evidence (a link OR a file — never neither).',
          'Contact point is structured FIELDS, not a template inside the description box. It is pre-filled from the pool row when there is one, and it is written to contact #1 on approval.',
          ...
          'Every row in the pool list ends in the action: Xin nhận on a free row, Xin nhận (đã có người xin) on a pending one, and the Company ID + “đã có sales phụ trách” on a claimed one. A row a rep cannot act on says why.',
          'Ghi chú duyệt is required on a rejection. Resolved requests stay in the queue, filterable by Trạng thái.',
          ...
          'A row already Đang chờ duyệt DOES accept further requests. The count is shown on the pool row, in the claim form and in the queue; approving one auto-rejects the others with “đã duyệt cho sales khác”.',
        ...
        backend: {
          ...
          endpoints: [
            ...
            'POST /admin/crm/company-pool/:id/claim { reason, freeDataKind, contactName, contactPhone, contactEmail?, evidenceUrl?, evidenceFileId? } — 409 if a CRM match exists; 422 if freeDataKind is one of the two customer values, or if neither evidence field is present. A row that is already pending is ALLOWED (competing request).',
            'GET /admin/crm/company-claims?status=pending',
            'POST /admin/crm/company-claims/:id/approve { note? } — creates the company, sets salesOwner = requestedBy, creates contact #1 from the contact point, sets pool state = claimed + claimedCompanyId, and auto-rejects every competing claim on that company with “đã duyệt cho sales khác”',
            'POST /admin/crm/company-claims/:id/reject { note } — note required; pool state returns to free if no other claim is pending',
        ...
        acceptance: [
          ...
          'Xin nhận cannot be submitted while lý do, phân loại, tên người liên hệ or SĐT is empty, or while both evidence fields are empty.',
          'Choosing either Saramin-package classification blocks submit and points the rep at the transfer flow.',
          'A pool row with a contact pre-fills the contact point on the form.',
          ...
          'Approving one of two competing requests auto-rejects the other with the note “đã duyệt cho sales khác”.',
          'A request with no evidence renders ⚠ không có bằng chứng in the queue; a very short reason renders in amber.',
          'Rejecting requires a Ghi chú duyệt; resolved requests remain listed and are filterable by Trạng thái.',
        ...
        openQuestions: [
          'The current FreeDB queue records TWO notes — **Ghi chú SA** and **Ghi chú Leader**. Saramin’s decision was "admin approves" (one approver), so this spec has one Ghi chú duyệt. Does Saramin need both roles to sign, and if so is Leader an approval step or only a comment?',
