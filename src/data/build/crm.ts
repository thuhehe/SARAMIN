          ...
          },
          {
            ...
            table: {
              ...
              rows: [
                ...
                ['**Yêu cầu nhận** tab (pool records only)', '—', 'The claim WORK: the two-level approval card, Phân trực tiếp, and Lịch sử yêu cầu nhận. Its own tab because **actions do not belong under a tab named “history”** — a mislabeled queue is a queue nobody finds. Conditional tabs have precedent: Applications/Resumes appear only for customers that have them. Amber badge while a request is open.'],
                ['Owner history tab', 'Owner tenure chain', '**Pure history on both kinds of record.** Pool: the kept chain with the release as its newest event, or “chưa có sales phụ trách” on a fresh import. Promoted CRM record: the chain whose first tenure IS the approved claim (“Nhận từ Free data — duyệt bởi {admin}”) plus a pointer to the log — the request-by-request detail lives there, not repeated on every promoted record.'],
