...
export const crm: BuildModule = {
  ...
  features: [
    ...
    // 5 · Sign-ups ────────────────────────────────────────────────────────────
    {
      ...
      scope: ['BE', 'FE'],
      ready: true,
      notes: 'Company-user sign-up lives here. A self-serve sign-up creates a LOGIN + an Unverified company shell so the person can explore immediately; this inbox is where HQ reconciles those shells against the existing company list — catching duplicates and junk (move the user + archive the empty shell).',
      mockup: 'crm-signups',
      detail: {
        requirements: [
        {
          label: 'Sign-up = a login + an Unverified company (self-serve)',
          text: 'Anyone can self-register on the Company site. The sign-up form captures the person (full name, email, phone, password) and their company (tax number, company name, "is your company currently hiring?" yes/no) plus a Terms/Privacy agreement, and on submit it creates a LOGIN and a NEW company (Unverified, no products) with that person as its Admin. They can then browse the whole employer site, but every data page (Candidates, Resume, Report…) is empty / locked with a Buy path until the company buys products and is verified.',
          table: {
            cols: ['Action', 'What it creates', 'Result'],
            rows: [
              ['Sign up (email + company name)', 'A login + a new company (Unverified, no products)', 'The signer-up is that company’s Admin; can browse, everything empty until they buy'],
              ['Sign up with an email already used', 'Nothing', 'Blocked — "this email already has an account, sign in instead"'],
              ['Create products / verify', 'Provisioned entitlements', 'Requires a business registration number, unique across verified companies'],
            ],
          },
          items: [
            ...
            'A company is Unverified until business registration is provided; posting a job or buying requires verification. Duplicate companies are prevented at verify/buy by a unique business registration number — NOT at sign-up.',
            'An empty self-created company (Unverified, no products, sole Admin) is a disposable shell: if that user is moved into or invited to a real company, the shell is abandoned/archived.',
          ],
          warn: 'A sign-up now DOES create a login + a company shell (this reverses the old "sign-up provisions nothing" model). What it never creates is products — those still come only from a paid order.',
        },
        {
          label: 'HQ reconciles sign-ups against the existing company list',
          text: 'Because anyone can self-register, this inbox is how HQ keeps the company list clean: each new self-serve company is matched against the companies we already have, and HQ resolves duplicates and junk. This is lead capture + de-duplication, performed AFTER the shell exists (not a gate before creation).',
          table: {
            cols: ['Case', 'Action', 'Result'],
            rows: [
              ['Completely new company — no match', 'Create on CRM (confirm the shell as a lead)', 'Enters the pipeline with customer status New; verifies at buy'],
              ['Matches an existing LEAD', 'Assign the user to that company + notify the lead’s owner + archive the shell', 'No duplicate; the lead’s rep knows a new contact self-registered'],
              ['Matches an existing CUSTOMER', 'Assign the user to that company + notify the account’s sales owner + archive the shell', 'The person becomes a user of the real paid account'],
              ['Public / free-mail email (gmail…) — can’t auto-match', 'Verify manually first, then pick one of the above', 'Guard against merging two unrelated companies on a shared free-mail domain'],
              ['Junk / fake / does-not-exist', 'Archive the shell', 'Removed from active lists, reversible, audited'],
            ],
          },
          items: [
            'Matching key order, strongest first: tax code (MST) → email domain → company name.',
            'Assigning a user into an existing company follows the Move-user rules (destination role chosen, seat cap respected) — see Account management.',
            'Edge case: two people self-register the SAME not-yet-in-CRM company (two shells). Reconcile to one — confirm one shell, move the second person into it, archive the extra shell.',
          ],
        },
        {
          ...
          table: {
            ...
            rows: [
              ...
              ['Archived', 'Removed from active lists, logins blocked, jobs hidden — record + audit kept', 'Reversible (Unarchive). An empty Unverified shell can be archived (or auto-archived after inactivity) freely; archiving a company with active products / other users / verified activity needs a reason and is confirmed. Never hard-deleted.'],
            ],
          },
          warn: 'To retire a duplicate: MOVE its users into the surviving company first, THEN archive the empty one — never archive a company that still has active users or paid products without resolving them.',
        },
        {
          label: 'Sign-ups status',
          text: 'The status of a row in the Sign-ups reconciliation inbox. Every self-serve sign-up already created a login + an Unverified company shell; the row tracks whether HQ has reconciled that shell against the existing company list.',
          table: {
            cols: ['Status', 'Means', 'Rule'],
            rows: [
              ['New', 'Unreconciled — the shell exists and has been matched, awaiting a disposition', '**System** creates the login + shell, opens the row and runs the match; **Sales** must disposition it. It stays New until an outcome is recorded.'],
              ['Resolved', 'An outcome was recorded, with a link to what happened', '**Sales** picks one: Genuine new → Keep (nurture the shell as a lead) · Matches a lead/customer → Move the user into it + archive the shell · Spam/junk → Archive the shell. **System** records the link.'],
            ],
          },
          warn: 'The sign-up already created a login + a company shell. Reconciliation never creates a SECOND company — for a duplicate it moves the user into the existing company and archives the shell.',
        },
        ],
        description:
          'The reconciliation inbox for people who self-register on the Company site. Each sign-up has already created a login + an Unverified company shell (so the person can explore); this screen is where HQ matches that shell against the companies we already have and resolves duplicates and junk — keeping a genuine new one, moving a duplicate’s user into the real company and archiving the shell, or archiving spam. It is the guard that keeps the company list clean now that anyone can self-create.',
        userStory:
          'As a sales/ops user, I want every self-serve sign-up matched against the companies we already have, so that a genuine new one is kept, a duplicate’s user is moved into the real account and the empty shell is archived, and spam is archived.',
        uiFields: [
          {
            group: 'Sign-up row',
            items: [
              ...
              { name: 'companyNameTyped', type: 'string', required: true, notes: 'the name the person entered at sign-up — already the shell company’s name' },
              { name: 'companyShellId', type: 'ref → Company', notes: 'the Unverified company created at sign-up — what a duplicate/junk disposition archives' },
              ...
              { name: 'matchResult', type: 'derived', notes: 'New company · Needs verification (public email) · Matches lead · Matches customer — with the matched record + confidence' },
              { name: 'receivedAt', type: 'timestamp' },
              { name: 'status', type: 'enum', notes: 'New → Resolved (Created on CRM / Assigned + archived) / Archived' },
            ],
          },
        ],
        behaviors: [
          'On sign-up the system creates the login + an Unverified company shell, then opens a reconciliation row and runs matching in a fixed key order — tax code (MST) → email domain → company name — showing which key matched.',
          'A public email domain (gmail, yahoo…) never auto-matches; the row is flagged for manual verification.',
          'The operator resolves each row with one action; the row then shows what happened and links to the record.',
          'Reconciliation never provisions products — a kept shell has no products until the normal PO → activation (or self-serve buy) flow runs.',
        ],
        rules: [
          'Three dispositions, one action each: Genuine new → Keep (nurture the shell as a lead, customer status New) · Matches an existing lead/customer → Move the user into it (role chosen) + archive the shell · Spam/junk → Archive the shell.',
          'Reconciliation never creates a second company; a duplicate is resolved by moving the user and archiving the empty shell.',
          'Moving a user into an existing customer follows the Move-user rules (destination role chosen, seat cap respected); the existing company’s Admin (or HQ break-glass) governs who ends up there.',
          'A row cannot be left half-resolved: it is New until an outcome is recorded.',
        ],
        states: ['New (unreconciled — shell created)', 'No match found', 'Matches a lead', 'Matches a customer', 'Public-domain email — needs manual check', 'Resolved (kept)', 'Resolved (moved + shell archived)', 'Archived as spam'],
        backend: {
          dataModel: [
            { name: 'signupId', type: 'uuid', required: true },
            { name: 'email', type: 'string', required: true },
            { name: 'companyNameTyped', type: 'string', required: true },
            { name: 'companyShellId', type: 'uuid', required: true, notes: 'the Unverified company created at sign-up' },
            { name: 'taxCode', type: 'string?' },
            { name: 'matchedCompanyId', type: 'uuid?', notes: 'set by matching, confirmed by a human' },
            { name: 'outcome', type: 'enum?', notes: 'kept | moved_and_archived | archived_spam' },
            { name: 'status', type: 'enum', required: true },
          ],
          endpoints: [
            'GET /admin/crm/signups?status=',
            'POST /admin/crm/signups/:id/keep — nurture the shell as a lead',
            'POST /admin/crm/signups/:id/move-and-archive { toCompanyId, role } — move the user into the matched company, archive the shell',
            'POST /admin/crm/signups/:id/archive { reason } — spam/junk',
          ],
          integrations: ['Company site sign-up form (source — creates login + shell)', 'CRM Companies (match + archive)', 'Account management (move user → company users)', 'Notifications (owner notice / move notice)'],
          notes: 'Matching is server-side and returns the key that matched plus a confidence. Uniqueness among VERIFIED companies is enforced on the business registration number at verify/buy, not here.',
        },
        acceptance: [
          'Signing up with a new email + company name creates a login and an Unverified company with that person as Admin; all data pages are empty until they buy.',
          'Signing up with an already-registered employer email is blocked with a "sign in instead" message — no second login or company.',
          'A sign-up with no match can be kept and nurtured as a lead (customer status New).',
          'A sign-up matching an existing lead/customer is resolved by moving the user into that company and archiving the shell — no duplicate company remains.',
          'A gmail/yahoo sign-up is never auto-matched and is flagged for manual verification.',
          'Every resolution (keep / move + archive / archive) is written to the audit log with the actor and the outcome.',
        ],
        openQuestions: [
          'Who owns the reconciliation inbox day to day — a named ops person, or the rep who owns the matched company?',
          'Should empty Unverified shells auto-archive after N days of inactivity, and what is N?',
          'When a user is moved into an existing customer, is the company’s Admin asked to approve, or does HQ place them directly?',
        ],
      },
    },
  ],
