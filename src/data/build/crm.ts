...
export const crm: BuildModule = {
  ...
  requirements: [
    ...
    },
    {
      ...
      text: 'Added 2026-09-20. Every document list carries the same period control at the TOP of the page — above the summary cards and the table: **Today · This week · This month · This quarter · YTD · All time · Time range**. YTD is the default: a sales team reads its year, and “all time” as a default grows into a list nobody scrolls. Choosing **Time range** reveals **From … To** date pickers, both inclusive. One window filters the rows, the status counts on the tab strip and the summary cards together, so the three can never disagree — and it is remembered while moving between the three lists, so “which period am I looking at?” is answered once.',
      table: {
        ...
        rows: [
          ...
          ['Time range', 'From → To, both inclusive, chosen by the user', 'From may not be after To — the pickers enforce it and say so. Switching into Time range pre-fills the pair with the preset that was just showing, never with two blanks'],
        ],
      },
      items: [
        ...
        'THE RESOLVED DATES ARE ALWAYS PRINTED beside the control (“01/01/2026 – 08/08/2026 · 9 báo giá”), so a preset never hides what it actually means and a screenshot of the page carries its own window.',
        ...
        uiFields: [
          ...
          },
          {
            ...
            items: [
              ...
              { name: 'period', type: 'filter · presets + range', required: true, notes: 'Today · This week · This month · This quarter · YTD (default) · All time · Time range (From – To). Sits on the page ABOVE the cards, not in the Filter panel; filters on the quotation’s CREATED date; rows, tab counts and cards all read it. Remembered across the three document lists. See the module block “Time period filter”' },
        ...
        uiFields: [
          ...
          },
          {
            ...
            items: [
              ...
              { name: 'owner', type: 'ref → admin user' },
              { name: 'period', type: 'filter · presets + range', required: true, notes: 'The shared control — Today … YTD (default) … Time range — on the PO’s ISSUE date. Tab counts (Active · Draft invoice · Invoice requested · Invoice issued · Expired) are counted inside the window. See the module block “Time period filter”' },
        ...
        uiFields: [
          {
            ...
            items: [
              ...
              { name: 'period', type: 'filter · presets + range', required: true, notes: 'The shared control on the invoice’s ISSUE date; a draft with no issue date follows its PO’s issue date. Tab counts are counted inside the window. See the module block “Time period filter”' },
