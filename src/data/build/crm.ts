...
export const crm: BuildModule = {
  ...
  requirements: [
    ...
    {
      label: 'Time period filter — one control on the three document lists (Quotations · Purchase order · Invoices)',
      text: 'Added 2026-09-20. Every document list carries the same period control at the TOP RIGHT of the page, above the summary cards and the table — ONE DROPDOWN BUTTON in the Google Analytics pattern. The button reads the preset and its resolved dates (“YTD · 01/01/2026 – 08/08/2026 ▾”); the panel lists the presets down the left — **Today · This week · This month · This quarter · YTD · All time · Time range** — with **Start date / End date** boxes on the right, and a change takes effect on **Apply** (Cancel, or clicking away, discards it). YTD is the default: a sales team reads its year, and “all time” as a default grows into a list nobody scrolls. One window filters the rows, the status counts on the tab strip and the summary cards together, so the three can never disagree — and it is remembered while moving between the three lists, so “which period am I looking at?” is answered once.',
      table: {
        cols: ['Preset', 'Resolves to', 'Note'],
        rows: [
          ['Today', 'Today, 00:00 → 23:59', ''],
          ['This week', 'Monday of the current week → today', 'Weeks start on MONDAY (VN convention), never Sunday'],
          ['This month', '1st of the current month → today', ''],
          ['This quarter', '1st day of the current calendar quarter → today', 'Jan–Mar · Apr–Jun · Jul–Sep · Oct–Dec'],
          ['**YTD** (default)', '1 January of the current year → today', ''],
          ['All time', 'No lower or upper bound', 'The only preset that also shows UNDATED rows — a draft invoice not yet filed'],
          ['Time range', 'Start → End, both inclusive, chosen by the user', 'The Start / End boxes are always visible and show the selected preset’s dates; typing in either one switches the selection to Time range — the move Google Analytics makes to “Custom” — so a preset can never sit selected beside dates it does not resolve to. Start may not be after End: the panel says so and Apply is disabled. Switching into Time range pre-fills the pair with the preset that was just showing, never with two blanks'],
        ],
      },
      items: [
        'WHICH DATE IS FILTERED — the document’s OWN date, and only that: the quotation’s **created** date, the PO’s **issue** date, the invoice’s **issue** date. A draft invoice has no issue date yet, so it follows its PO’s issue date rather than vanishing from every preset but All time. Expiry, payment and activation dates stay COLUMNS — narrowing on them is a different question, asked through Filter, not through the period.',
        'THE PERIOD IS NOT A ROW IN THE FILTER PANEL. It sits on the page, above the numbers, because it changes what every number on the page MEANS — a control that moves the denominator cannot live in a dropdown beside Status.',
        'SUMMARY CARDS READ THE PERIOD, from the same filtered rows the table shows. Quotations: count · total value · sent to customer · converted to PO (with the conversion share). Purchase order: count · total · collected · outstanding. Invoices: count · total · officially issued · collected. All recompute on every change of the window.',
        'TABS AND FILTER STAY INSIDE THE PERIOD — the status counts on the tab strip are COUNTED within the window, never typed, so “Active 9” means nine in this period and cannot mean nine ever.',
        'THE RESOLVED DATES ARE PRINTED ON THE BUTTON ITSELF (“YTD · 01/01/2026 – 08/08/2026”) and the row count sits on the same line (“9 báo giá trong kỳ”), so a preset never hides what it actually means and a screenshot of the page carries its own window.',
        'NOTHING MOVES UNTIL APPLY. The panel edits a draft; the cards and the table keep the window that was showing until Apply is pressed. A list that re-sorts under the cursor while someone is still choosing a date is the behaviour every analytics tool has trained people out of expecting.',
        'TODAY IS THE SERVER’S DAY IN VIETNAM TIME (UTC+7), not the browser’s. A rep travelling, or a reviewer in Korea, must see the same “Today” as the office; and the window is applied in the query, not by trimming a page of results in the browser.',
        'A WINDOW THE DATA CANNOT FILL IS NOT AN ERROR. An empty table with the window restated is the correct result, and the presets are one click away — never auto-widen to the nearest non-empty period.',
        'THE SAME CONTROL IS THE PATTERN for every later list or report with dated rows (Payments, Pipeline, the Sales report). It is defined once in the shared toolbar kit and never restyled per page — the one filter-and-sort convention across all tables.',
      ],
    },
  ...
  features: [
          ...
          },
          {
            ...
            table: {
              ...
              rows: [
                ...
                ['Product usage (company site)', 'One group per shared invoice: invoice number (INV-…) + “Shared by {Sponsor}” — no date/amount/PO; rows are counts only, no bar; Post job / Find talent, no Activate', 'Its page as today + the usage matrix above the list (same table as the admin record, no Link / Remove); bars include beneficiaries’ use', 'n/a — see Products & billing'],
        ...
        uiFields: [
          ...
          },
          {
            ...
            items: [
              ...
              { name: 'period', type: 'dropdown · presets + Start/End', required: true, notes: 'One button at the top right (Google Analytics pattern) reading the preset and its dates; panel = presets on the left (Today · This week · This month · This quarter · YTD (default) · All time · Time range), Start / End dates on the right, Cancel / Apply. Sits on the page ABOVE the cards, not in the Filter panel; filters on the quotation’s CREATED date; rows, tab counts and cards all read it. Remembered across the three document lists. See the module block “Time period filter”' },
              { name: 'summary cards', type: 'derived × 4', notes: 'Báo giá · Tổng giá trị (highest option per quotation, summed) · Đã gửi khách · Chuyển thành PO (+ share). Computed from the period-filtered rows — never from the whole table' },
            ],
          },
        ],
        behaviors: [
        ...
        states: [
          ...
          },
          {
            ...
            items: [
              ...
              'The PO copies the accepted option’s figures as they stand. The INVOICE keeps the totals but re-expresses the lines — it never shows a discount, so every discount is folded back into the unit price first; see “Từ báo giá sang hóa đơn” below. Neither document recomputes from the catalogue — prices change, and a filed invoice must not move when they do.',
            ],
          ...
          {
            label: 'Từ báo giá sang hóa đơn — hóa đơn không bao giờ hiển thị chiết khấu',
            text: 'The quotation and the invoice are **two different arithmetics with one total**. The quotation shows every discount: a Giảm column per line, then the order-level percentage and the fixed amount as their own rows. A VAT invoice shows **none** of them — it prints đơn giá × số lượng = thành tiền and nothing else. So before the invoice is written, every discount the quotation carried is folded back **into the unit price**.\n\nThe method is the one the client’s current CRM already runs (`Invoice.php::getDetailsItem()`, documented in “Từ Invoice → Hoá đơn điện tử”): take the **three frozen totals** — tổng trước thuế, tổng tiền thuế, tổng thanh toán — and **split them down** onto the lines by value, the last line taking every remainder. Nothing is rebuilt from the lines up.\n\nThis is not a display choice. The e-invoice provider’s template has no discount column, the tax authority reads đơn giá and thành tiền, and a customer who sees “−12%” on a quotation and “1.608,50” on the invoice must still land on the same 11.470 ₫.',
            figure: 'quote-to-invoice',
            table: {
              cols: ['#', 'Step', 'Formula', 'Worked example (A 2.000 × 2 −12% · B 3.000 × 3 −10%)'],
              rows: [
                ['1', 'Line net, to the đồng', '`net_i = round(SL × đơn giá × (1 − CK dòng))`', 'A = 3.520 · B = 8.100'],
                ['2', 'Subtotal', '`sub = Σ net_i`', '11.620'],
                ['3', 'ONE order-level lump', '`lump = round(sub × CK tổng đơn %) + giảm số tiền`', '581 + 419 = **1.000**'],
                ['4', 'The three FROZEN totals', '`base = sub − lump` · `vat = round(base × VAT%)` · `total = base + vat`', '**10.620 · 850 · 11.470** — fixed from here on; everything below only splits them'],
                ['5', 'Pre-tax per line, every line but the last', '`invNet_i = round(base × net_i / sub)`', 'A = round(10.620 × 3.520 / 11.620) = 3.217,07 → **3.217**'],
                ['6', 'The LAST line takes the remainder', '`invNet_last = base − Σ invNet_others`', 'B = 10.620 − 3.217 = **7.403**'],
                ['7', 'VAT per line — allocated, not recomputed', '`invVat_i = round(vat × invNet_i / base)`, last line takes the remainder', 'A = round(850 × 3.217 / 10.620) = **257** · B = 850 − 257 = **593** (not 592 = 7.403 × 8%)'],
                ['8', 'Invoice đơn giá — decimal, presentation only', '`invUnit_i = invNet_i / SL`', 'A = 1.608,50 · B = 2.467,67'],
                ['9', 'What prints', 'thành tiền · tiền thuế · tổng per line; the three totals in the summary', 'A 3.217 · 257 · 3.474 — B 7.403 · 593 · 7.996 — Σ **10.620 · 850 · 11.470**'],
              ],
            },
            items: [
              '**Line discounts go into their own line.** A 12% on Sản phẩm A becomes a unit price of 2.000 × 0,88 = 1.760 before any order-level discount is considered. The order-level lump is then spread on top of these already-discounted nets, never on list prices.',
              '**Order-level % and fixed ₫ are ONE lump.** They are summed first (581 + 419 = 1.000) and allocated together. Allocating them separately produces two rounding remainders instead of one, and a total that can miss the quotation by a đồng.',
              '**Every “last” absorbs the rounding.** Pro-rata shares rarely sum to their total (3.217,07 + 7.402,93). Rounding each and giving the last line `total − Σ others` is what guarantees Σ thành tiền = tổng trước thuế and Σ tiền thuế = tổng tiền thuế exactly. Which line is “last” must be deterministic — the printed order.',
              '**Per-line VAT is allocated, never recomputed.** `rate × thành tiền` per line gives 257 + 592 = 849 against a total tax of 850, and the provider’s rate-summary block would then disagree with its own lines. Split the frozen tổng tiền thuế by pre-tax value, last line takes the remainder: 257 + 593 = 850.',
              '**Mixed VAT rates (5% · 8% · 10% · không thuế) — split by rate GROUP first.** Each group gets `round(group pre-tax × rate)`; the **last taxable group in printed order** takes the remainder; non-taxable lines get 0 and are never “last”. Then split each group’s tax across its lines by pre-tax, last line in the group taking the remainder. This is what makes the “tổng hợp theo thuế suất” block on the PDF equal rate × tiền hàng for every group. One rate today, but the rule has to be in the build before the first 5% line appears.',
              '**Thành tiền is the fact; đơn giá is presentation.** The unit price is derived from the integer net and shown to 2 decimals. It must **never** be multiplied back out to recompute thành tiền: 2.467,67 × 3 = 7.403,01, not 7.403. A dev who stores the rounded unit price and recomputes the line total will be one đồng off on a third of invoices, and every one of those is a filed fiscal document.',
              'Decimal đơn giá is allowed on a VN e-invoice (the XML schema takes a decimal; thành tiền and the totals are whole đồng). If the client’s provider rejects decimals, the fallback is to carry the remainder in the last line’s thành tiền and accept that đơn giá × SL ≠ thành tiền by a few đồng on that one line — confirm with the provider before build.',
              'Gift lines (0 ₫) have net 0, share 0 and print at 0 — the formula needs no special case for them.',
              'The PO is NOT re-expressed. It is a commercial document and prints the quotation’s lines as they stand, discounts included. Only the invoice folds them in.',
            ],
            warn: 'Fields for build, on the invoice line: `netAmount` (int ₫ — the fact), `allocatedOrderDiscount` (int), `unitPriceNet` (decimal 2, derived, never the source of anything). The invoice total is asserted equal to the quotation’s “Sau chiết khấu” + VAT at write time; a mismatch is a bug, not a rounding tolerance.',
          },
          {
        ...
        uiFields: [
          ...
          },
          {
            ...
            items: [
              ...
              { name: 'owner', type: 'ref → admin user' },
              { name: 'period', type: 'dropdown · presets + Start/End', required: true, notes: 'The shared dropdown — Today … YTD (default) … Time range, Apply to take effect — on the PO’s ISSUE date. Tab counts (Active · Draft invoice · Invoice requested · Invoice issued · Expired) are counted inside the window. See the module block “Time period filter”' },
              { name: 'summary cards', type: 'derived × 4', notes: 'PO · Tổng giá trị (after VAT) · Đã thu tiền (count + amount) · Chưa thu (outstanding amount + count). From the period-filtered rows' },
            ],
          },
        ],
        behaviors: [
    ...
    // 4 · Invoice ─────────────────────────────────────────────────────────────
    {
      ...
      detail: {
        ...
        {
          label: 'Dòng hàng trên hóa đơn — đơn giá đã gồm chiết khấu',
          text: 'An invoice line is **đơn giá × số lượng = thành tiền**, and that is the whole line. There is no discount column and no discount row, because the provider’s template has none and the tax authority reads none. Every discount the quotation gave — per line, order-level %, fixed amount — has already been folded into the unit price by the time the invoice exists.\n\nThe arithmetic lives on the Quotation page under “Từ báo giá sang hóa đơn”; this block is what the invoice screen and the PDF must obey.',
          figure: 'quote-to-invoice',
          items: [
            'One invoice line per quotation line, in the quotation’s order. No line is merged, split or re-described — the customer matches them by eye.',
            '**Thành tiền is the stored integer.** **Đơn giá is derived** from it and shown to 2 decimals. The screen and the PDF print the stored thành tiền; they never recompute it from the displayed unit price.',
            'Cộng tiền hàng = Σ thành tiền = the quotation’s “Sau chiết khấu”, to the đồng. Tổng tiền thuế is computed once on that sum and then **allocated** to the lines (pro-rata by pre-tax, last line takes the remainder) — so the per-line VAT in the provider’s columns 7–9 sums to the total exactly: 257 + 593 = 850, never 257 + 592 = 849. With mixed rates the split goes by rate group first; see “Từ báo giá sang hóa đơn” on Quotations.',
            'A draft invoice prints the same lines the official one will. The fold-in happens when the draft is created, so nothing about the numbers changes when Kế toán makes it official — only the number, the signature and the tax authority code appear.',
          ],
        },
        {
        ...
        uiFields: [
          {
            ...
            items: [
              ...
              { name: 'period', type: 'dropdown · presets + Start/End', required: true, notes: 'The shared dropdown, on the invoice’s ISSUE date; a draft with no issue date follows its PO’s issue date. Tab counts are counted inside the window. See the module block “Time period filter”' },
              { name: 'summary cards', type: 'derived × 4', notes: 'Hoá đơn (archived excluded) · Tổng giá trị · Đã xuất chính thức (+ drafts / requested) · Đã thu tiền (count + amount). From the period-filtered rows' },
            ],
          },
        ],
        behaviors: [
