    ...
    },
    {
      ...
      table: {
        ...
        rows: [
          ...
          ['9', 'Kế toán only', 'Cancel the **invoice**, if it went out first and the money never came', 'Invoice withdrawn · quota clawed back. The PO itself is never cancelled'],
  ...
  features: [
        ...
        states: [
        ...
        ],        requirements: [
          {
            ...
            table: {
              ...
              rows: [
                ['**Existing**', 'Chiết khấu theo số lượng', 'Sums the quantity of each **product** across the option (“cùng loại”) and sets every line of that product from the total — 2+ → 25%, 5+ → 30%, 10+ → 35%, 20+ → 40%, 30+ → 45%, 50+ → 50%, 100+ → 60%. A total of 1 gets 0%. The tiers are thresholds: 7 earns 30%, and 3 + 4 on two lines is 7.'],
                ['**New** or **Churn**', 'Giảm 50% tất cả dịch vụ', 'Sets the option-level discount to 50% — but only while **every** non-gift line is ≤ 5. One line at 6 and the whole 50% is gone, including from the lines that were within the cap. First PO of the current status spell only, so a Churn customer coming back qualifies again.'],
            ...
            items: [
              ...
              'Gift (“Tặng”) lines take no discount, do **not** count toward the quantity cap, and are **not** added into the per-product sum. Adding a gift must never destroy the customer’s 50%, nor push a product into a higher tier.',
              ...
              'A rate the programme granted is **exempt** from the >20% approval gate — it was approved when the programme was configured, and every tier from 25% up would otherwise route. Approval fires only once a rep switches auto-apply off and types their own number, so a pending approval always means somebody deviated from policy.',
    ...
    // 3 · Purchase order ──────────────────────────────────────────────
    {
      ...
      detail: {
        requirements: [
        {
          label: 'PO status — three statuses, only one of them a step',
          text: 'The status carried on the PO record and the PO list. A PO is created from exactly one accepted quotation option, carries that option’s lines forward unchanged, and is **Active** from the moment it is issued — there is no draft and no unsent state.\n\nOnly one action ever moves a PO forward: **Kế toán** issuing the VAT invoice on it. The third status is an exit nobody clicks (Expired).\n\nThere is deliberately **no Cancelled**. Undoing a sale means withdrawing the **invoice** — that is the fiscal document, and it is what granted the quota. See the block below.',
          table: {
            ...
            rows: [
              ...
              ['Issued invoice', 'A VAT e-invoice has been issued against this PO', '**Kế toán only**', 'The consequential status. Issuing provisions the products **immediately** — see the rule below. The deal closes, the customer leaves Prospect and the 12-month activation clock starts. Terminal on the PO: any correction happens on the invoice.'],
          ...
          items: [
            'There is no cancel on a PO at all. An Active PO that goes nowhere simply **expires** at the end of the month, and an invoiced one is undone by cancelling the **invoice**.',
            'Payment is **not** a status. It is a fact recorded against the PO (paymentState: unpaid · partially paid · paid), because the invoice may legitimately precede it. A PO can therefore be “Issued invoice” and still unpaid — that is a receivables problem for **Kế toán**, never “Lost”.',
            'Draft and Sent were removed together: a PO comes into existence **by** being issued, so no status separated “written” from “sent”. Paid was removed because it no longer gates anything. Cancelled was removed because the invoice already carries it.',
          ],
          warn: 'Deviation from the client T&C to confirm. Clause 3 says the service activates after payment **and** invoice; this model releases the product on the invoice alone, so a customer who has not yet paid can already post jobs and open CVs. The recovery is an invoice cancellation, which may have to claw back quota already consumed. Decide the claw-back rule before build: refuse to cancel once any quota is used · cancel and let the balance go negative · cancel and settle on the credit note.',
        },
        {
        ...
        states: ['Đang hiệu lực / Active — won', 'Đã xuất hóa đơn / Issued invoice', 'Hết hạn / Expired — end of month'],
        sections: [
          {
            heading: 'Mô tả tổng quát các trạng thái — four statuses, one forward step, two exits',
            items: [
              'three statuses, but only **one forward step**: Active → Issued invoice. The third is an exit. The screen shows the current status and at most one action; the full model below is the reference, not something restated on the page.',
              ...
              '2 · Đã xuất hóa đơn / Issued invoice — the VAT e-invoice exists with its legal number. The deal closes, customer status flips out of Prospect, the 12-month activation window opens, and **the products land on the customer’s account in the same moment** — they can post a job or open a CV immediately. → Terminal on the PO. Any correction is made on the invoice.',
              ...
              '**Removed** — “Đã hủy / Cancelled”. It only ever meant “the invoice against this PO was withdrawn”, which the invoice already records. Two cancel paths for one event give two records that can disagree about whether the sale happened, and only the invoice is the document the tax office recognises. An Active PO needs no cancellation either — it expires.',
        ...
        backend: {
          dataModel: [
            ...
            { name: 'status', type: 'enum', required: true, notes: 'active|invoiced|expired. active→invoiced is the only forward transition and active→expired is a job, not a call. There is no cancelled value: withdrawing a sale is an invoice operation. Validated server-side, never only by the UI' },
          ...
          endpoints: [
            ...
            '(no /cancel on orders — cancellation is POST /admin/crm/invoices/:id/cancel)',
          ],
          integrations: ['Quotations (source)', 'Invoices + Payments (downstream)', 'Account management (provisioning target, after payment)'],
          notes: 'Confirm launch scope — real svn-be build if yes. Emits order.issued (pipeline → PO / won) and order.expired. Provisioning listens to invoice.issued and the claw-back to invoice.cancelled — never to an order event.',
        },
        acceptance: [
    ...
    // 4 · Invoice ─────────────────────────────────────────────────────────────
    {
      ...
      detail: {
        ...
        rules: [
          ...
          'The PO must be Active. An expired PO cannot be invoiced, and this is enforced server-side rather than by disabling a button.',
    ...
    // 5 · Sign-ups ────────────────────────────────────────────────────────────
    {
      ...
      detail: {
        ...
        description:
          'The inbox for people who self-register on the Company site. A sign-up clears two gates before login: (1) email verification — sent immediately, self-serve; (2) HQ placement — an operator, seeing only email-verified rows, resolves each with one of three actions: move the user into an existing company, create a new company and move them in as Admin, or archive. Move/Create unlock login and send a "you’re in" email. The user is kept informed by a status tracker and a stated SLA throughout the (slower) placement step.',
        userStory:
          'As a sales/ops user, I want every email-verified sign-up matched against companies we already have and resolved with one action, so that a real one lands in the right company (existing or new) and spam is discarded — while the user always knows where they are in the process.',
        uiFields: [
          {
            group: 'Sign-up row',
            items: [
              ...
              { name: 'emailVerified', type: 'bool', notes: 'gate 1 — set true when the user clicks the verification link. HQ can only action verified rows.' },
              ...
              { name: 'status', type: 'enum', notes: 'New (awaiting verify → verified) → Resolved (Moved to existing / Created + moved) / Archived' },
            ],
          },
        ],
        behaviors: [
          ...
          'Gate 2 — HQ placement: an operator resolves each EMAIL-VERIFIED row with one of the same three actions — Move to existing · Create new company + move · Archive. An unverified row is shown but not actionable ("awaiting email verification").',
        ...
        rules: [
          ...
          'Three actions, one per sign-up, the SAME regardless of match: Move to existing · Create new company + move · Archive. HQ may only act once email is verified.',
          'Creating a new company makes the user its Admin; moving into an existing company follows the Move-user rules (role chosen, seat cap respected) — see Account management.',
        ...
        states: ['Pending email verification', 'Email verified · pending review', 'Match (tax hit)', 'Not match', 'Public-domain email — verify manually', 'Resolved (moved to existing)', 'Resolved (new company created)', 'Archived / rejected', 'Active (both gates passed)'],
        backend: {
          dataModel: [
          ...
          endpoints: [
            ...
            'POST /admin/crm/signups/:id/move-to-existing { companyId, role } — verified only; unlock login; send "you’re in" email',
            'POST /admin/crm/signups/:id/create-and-move { companyName, taxCode } — verified only; user = Admin; send "you’re in" email',
