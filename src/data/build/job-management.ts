  ...
  features: [
       ...
       requirements for one form is how the field list drifts apart. */
    {
      ...
      detail: {
        ...
        rules: [
          'COMPANY SITE — an UNVERIFIED company cannot post. Both **Publish** and **Save draft** are disabled, and the page says why in one sentence with the fix attached: “Công ty chưa được xác minh — chưa đăng tin được, kể cả bản nháp. Để được xác minh, hoàn tất 3 mục ở Company information” → the three inputs Saramin verifies against, each ✓ or ✗ — **MST · địa chỉ đăng ký MST · Giấy chứng nhận đăng ký doanh nghiệp (ERC)** — and a button to Company information. All three in: “Đủ hồ sơ — Saramin xác minh trong 1 ngày làm việc; sau đó bạn đăng tin được ngay.” The list is rendered by the same rule the admin’s Customers filter uses (readyToVerify), never a hand-written sentence. The employer can still open the form and read it. Verification is the ONE gate on posting: once the company is Verified a draft needs no invoice, no PO and no product — see CRM → Sign-up & company verification (ERC). ADMIN SIDE is not gated: HQ may still post on behalf of any company (concierge), verified or not.',
