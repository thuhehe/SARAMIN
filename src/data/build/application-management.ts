    ...
    // 0 · Apply ───────────────────────────────────────────────────────────────
    {
      ...
      detail: {
        ...
        openQuestions: [
          ...
          'REOPENED (2026-08-09) — the 2026-08-05 cut was REVERSED on client direction: nationality, gender, marital status and date of birth ARE collected again, and appear in Basic information on My CVs, Create CV and the apply read-back. Recording the trade so it is a decision and not a drift: none of the four is read by CV search or by the CV↔JD match, so they add profile length without adding findability, and MARITAL STATUS in particular is the field most likely to be challenged on discrimination grounds in a hiring context. Recommend at minimum: all four optional, never required to apply, and marital status reviewed with the client’s legal side before launch.',
