    ...
    // 0 · Apply ───────────────────────────────────────────────────────────────
    {
      ...
      detail: {
        ...
        openQuestions: [
          ...
          'REOPENED (2026-08-09) — the 2026-08-05 cut was REVERSED on client direction: nationality, gender, marital status and date of birth ARE collected again, and appear in Basic information on My CVs, Create CV and the apply read-back. UPDATED 2026-08-10: two of the four now have a consumer — GENDER and DATE OF BIRTH (as an age) are shown on the employer CV-search row and are proposed as filter facets (see Resume management → Resume list). That answers the "adds length without adding findability" objection for those two, and sharpens the discrimination one: a field that is merely stored is a privacy question, a field an employer can FILTER a pool by is a discrimination question. NATIONALITY and MARITAL STATUS still have no reader anywhere in the product. Recommend: all four optional and never required to apply; gender + age facets reviewed and signed off by the client’s legal side before launch; marital status reconsidered for cutting, since nothing reads it and it is the single most challengeable field in a hiring context.',
