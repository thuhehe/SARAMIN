        ...
        sections: [
          {
            ...
            items: [
              ...
              '★ UNBLOCK IS SIMPLE, BECAUSE BLOCK WAS. It returns sign-in and nothing else — there is nothing to resurrect, because nothing was taken. Reinstate puts the account back to whatever it was BEFORE (Active, or Pending verification), never blindly Active: a suspension must not launder an unverified email into a verified one.',
              '★ IF THE CV WAS REJECTED ALONGSIDE THE BLOCK, UNBLOCKING DOES NOT UNDO THAT. Two decisions were taken, so two have to be reversed — and the CV one is reversed on the CV, by Approve, which is exactly how any other rejection is undone. An unblock that silently re-approved CVs would be the cascade coming back through the other door.',
              'A CV THAT WAS NEVER REJECTED NEVER LEFT SEARCH, so there is nothing to re-enter — the block did not remove it. Where a rejection IS later reversed, the CV comes back as an ordinary row with no notification: an employer pinged about a candidate they were told to ignore is confusing at best.',
              'NO UNLOCK IS REFUNDED ON A BLOCK, because nothing was withdrawn from the employer. Refunds belong to the CV-rejection path, where a credit really did buy something we then took away.',
              'BLOCK AND UNBLOCK ARE BOTH AUDITED with a mandatory reason, on the user record — not in CV review, which only ever holds CV decisions. Keeping the two logs apart is the same separation as the two levers; a wrong-block pattern is read on the user record.',
              'A BLOCKED ACCOUNT CAN STILL BE DELETED — by HQ on a legal request, or by the candidate if they can still reach the flow (they cannot sign in, so in practice it is HQ). Deletion is FINAL and there is no unblock afterwards, so an account that might be reinstated must not be deleted to tidy up a queue.',
