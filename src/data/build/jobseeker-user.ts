        ...
        sections: [
          {
            ...
            table: {
              ...
              rows: [
                ...
                ['Applications already delivered', 'STAY with the employer', '**STAY** — a block does not touch them', 'STAY with the employer, tagged'],
                ['CVs the employer already UNLOCKED', 'Stay readable', '**Stay readable** — a block does not touch them either', 'Stay readable — the employer paid for that copy and keeps it'],
                ['What the employer SEES on those rows', '— (nothing changes)', '— (nothing changes; a block is invisible to them)', 'The row stays, tagged **“Tài khoản này không còn tồn tại”**'],
                ...
                ['Still in CV SEARCH?', 'No — out immediately', '**YES, unless the CV is also rejected** — the account is locked, the CV is not', 'No — out immediately. Only already-unlocked copies survive'],
              ],
            },
            items: [
              ...
              '★ A BLOCK IS ACCESS CONTROL AND NOTHING ELSE (decided 2026-08-23) — the single most important line on this page. It answers ONE question: can this person sign in? It does not reject their CVs, recall their applications, hide unlocked copies or take them out of CV search. An operator who wants those too sets the CV status to Rejected, which is a separate act on a separate screen with its own audit line. One lever, one meaning — and the practical reason is that a lever which does two things can never be used to do only one of them.',
              'SO A BLOCK IS INVISIBLE TO EMPLOYERS. Nothing on their side changes, which is correct: the person cannot log in, but nothing has been decided about the quality of what they already sent, and an employer mid-interview with a good candidate should not lose them because of an account-level matter that is none of their business.',
              '⚠ THE GAP THAT CREATES, and the UI has to close it: an abusive user is blocked and their CV is STILL IN SEARCH, their spam applications STILL in employer pipelines. That is the honest cost of the separation. The fix is not a cascade — it is a REMINDER on the block dialog: “Chặn đăng nhập không gỡ CV. Hồ sơ này vẫn đang hiển thị và có N đơn đang ở phía NTD — từ chối CV?” with a link to do it. Prompt, never automatic: the operator decides both, and knows they are two decisions.',
              'DELETE IS THE ONLY ONE THAT TOUCHES THE EMPLOYER SIDE, and only by adding a tag. The candidate exercised their own right; it is not the employer’s fault, and an employer who already received an application or paid a credit to unlock a CV keeps it. The row stays fully workable and simply gains **“Tài khoản này không còn tồn tại”** — which tells the recruiter the one thing they need, that replying is pointless.',
              'THE TAG IS NOT A RECALL. A recalled row is struck through, its CV blurred and its actions removed; a deleted-account row is a NORMAL row that happens to carry a tag. The recruiter can still read the CV, still see the application in their pipeline, still act on their own records — they simply learn that replying is pointless, which is the one thing the tag is for.',
              'THE EMPLOYER IS NEVER TOLD *WHY*. A block shows them nothing at all; a deletion shows a neutral tag that names no reason. Spelling out that a user was blocked would be both a judgement about a person and a hint to whoever is probing our abuse rules — and in the block case there is nothing to tell them anyway.',
              ...
              '★ WHAT UNBLOCK RESTORES — AND WHAT IT CANNOT. Reinstate returns the LOGIN and the CV, but it does NOT resurrect recalled applications. Recall is terminal by design: the employer was already told “Saramin đã thu hồi CV này”, and un-recalling would mean going back a second time to say we changed our mind — which costs more credibility than it returns. The candidate applies again, to whichever of those jobs are still open. This is not a gap; it is the price of having recalled at all, and it is why blocking is a decision to take deliberately rather than as a holding action.',
              '★ UNBLOCK RESTORES THE CV’S PREVIOUS STATUS, NEVER BLINDLY “Qualified”. The block set it to Rejected with reason = violation, which is a verdict about the ACCOUNT rather than about the document — so lifting the block must put back whatever the CV was before: Qualified, or a doubt state still awaiting review. The same rule the account itself already follows (“Reinstate returns it to Active or Pending verification, not blindly Active”), for the same reason: a suspension must not launder an unverified email or an unreviewed CV into a clean one.',
              'IF THE PRE-BLOCK STATUS WAS QUALIFIED, the CV re-enters CV search on unblock — and it re-enters as a NEW row for employers, not as the one that was withdrawn. Nobody is told “that candidate you lost is back”; there is no notification, because an employer being pinged about a candidate they were told to ignore is confusing at best.',
              'THE REFUNDED UNLOCK IS NOT RECLAIMED on unblock. The employer got their credit back when we withdrew the CV; taking it away again because we reversed our own decision would charge them for our mistake. If they want the CV again, they unlock it again — and most will not, which is the real cost of a wrong block.',
              'AN UNBLOCK LEAVES A TRAIL. The rejected-as-violation CVs stay visible in CV review’s Đã từ chối view with the reversal recorded, because two wrong blocks in a month is a pattern worth seeing and it is invisible if the evidence is deleted on reinstate.',
              'A BLOCKED ACCOUNT CAN STILL BE DELETED — by the candidate, whose right does not lapse because they are suspended, or by HQ on a legal request. Nothing is left to recall (the block already did it), so the only visible change is the tag on rows the employer kept. Deletion is FINAL: there is no unblock afterwards, so an account that might be reinstated should not be deleted to tidy up a queue.',
              'THE EMAIL FREES UP ONLY AFTER RETENTION EXPIRES. While a deleted record is still held for the legal period its email cannot start a new account — otherwise the “deleted” person and the new one collide on the one key everything joins on. A suspended account never frees its email at all; the record is intact and the address still belongs to it.',
              'WHAT IS STILL OPEN, and it is a legal question rather than a product one: how long an already-unlocked CV may stay readable to an employer after the candidate deletes. The employer paid for that copy and has almost certainly saved it elsewhere, so hiding it is largely theatre — but “forever” sits badly beside a deletion right under NĐ 13/2023. Recommend a retention window on the unlocked copy, agreed with legal, rather than either extreme.',
            ],
          },
          {
