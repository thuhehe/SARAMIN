...
export const resumeManagement: BuildModule = {
  ...
  requirements: [
    ...
    },
    {
      ...
      items: [
        'NO ACCEPTANCE GATE ON CONTENT — superseded. An earlier draft refused a file whose parse found no work experience and no education; that rule is GONE. The site never blocks an upload or an apply: whatever the candidate uploads is saved as a CV and is usable immediately. A blank page, an image-only scan or an unrelated document is caught at UPLOAD by the qualification rule, which holds it out of CV SEARCH only — the file is still saved and still applied with (see “CV qualification — one rule, two sources”). Only TYPE and SIZE are still checked at the picker, because those are facts about the file rather than a judgement of its contents.',
        ...
        'Two tiers of missing fields in Review, and NEITHER stops the candidate saving: the RED tier is the two APPLY-ELIGIBLE fields (≥ 1 work experience — or ≥ 1 education entry for a fresher — and ≥ 3 skills), which decide whether a SARAMIN CV can be sent to an employer; the RECOMMENDED tier is the ranking boosters (more skills, languages, desired salary) shown with an impact hint. Desired title and visibility consent are NOT review fields — the first is Work preference on the Profile, the second is one account-level switch.',
  ...
  features: [
    ...
    },
    {
      ...
      ready: true,
      mockup: 'admin-resumes',
      notes: 'ONE rule decides whether any CV — typed or uploaded — may be applied with and may enter CV search. Not a screen of its own: it writes a status that My CVs, the apply modal and the Admin Resumes list each render.',
      detail: {
        description:
          'There are TWO kinds of CV: an uploaded PDF, and a Saramin CV. A Saramin CV is fields the candidate typed. An uploaded PDF is parsed into exactly the same fields. So ONE rule reads both.\n\nWhat differs is only what happens when a CV FAILS that rule — and that is the single thing this page exists to pin down.',
        userStory:
          'As a jobseeker, I want to know exactly what my CV needs before I can apply with it or be found by employers, and never to be stopped by something Saramin got wrong.',
        sections: [
          {
            early: true,
            heading: 'In one table',
            text: 'Two kinds of CV, one rule, and the only thing that differs is what a failure costs.',
            table: {
              cols: ['', 'Saramin CV (typed)', 'Uploaded PDF (parsed into the same fields)'],
              rows: [
                ['The rule', '≥ 1 work experience (or an education entry) AND ≥ 3 skills', 'The same rule, on the extracted fields'],
                ['Passes', 'Apply ✅ · CV search ✅', 'Apply ✅ · CV search ✅'],
                ['Fails', 'Apply ❌ · CV search ❌ — not usable at all', 'Apply ✅ — but delivery waits · CV search ❌'],
                ['Who fixes it', 'The candidate, in the editor', 'Admin decides; or the candidate adds the missing skills'],
                ['Admin involved', 'Never', 'Yes — CV check queue'],
              ],
            },
            items: [
              'Admin approves the CV, NEVER the application. Approving sends every application waiting on that CV and makes the CV searchable, in one action.',
            ],
          },
          {
            early: true,
            heading: 'The rule — the same for both types',
            text: 'Arithmetic over fields we already hold. No scoring, no threshold to tune, no percentage.',
            table: {
              cols: ['Must have', 'Why'],
              rows: [
            ...
            items: [
              'It reads the CV’s FIELDS, not where they came from — extracted or typed by hand counts the same. So a thin extraction is something the candidate can fix in seconds.',
              'NEVER A PERCENTAGE — a score rewards photo, About and awards, so a CV could hit 70% with an empty Experience.',
            ],
          },
          {
            early: true,
            heading: 'CV status — ONE set, with the actions that change it',
            text: 'A CV has exactly one status at a time. Everything an operator or a candidate can do to a CV is in this table, with the status it produces — there is no second status column anywhere, and HQ moderation is not a separate axis but two of these values.',
            table: {
              cols: ['Status', 'What it means', 'Apply · delivery', 'CV search', 'Actions available → resulting status'],
              rows: [
                [
                  'Qualified',
                  'Passes the rule: ≥1 work experience (or education) and ≥3 skills.',
                  'Sent immediately',
                  '✅ indexed',
                  'HQ: Remove from pool → Removed. Candidate: edit below the rule → Not enough information.',
                ],
                [
                ...
                [
                  'Removed',
                  'HQ pulled the candidate out of the pool — fraud, abuse, duplicate accounts. Nothing to do with CV quality.',
                  'Unaffected — this is about SEARCH, not applying',
                  '❌',
                  'HQ: Restore → Qualified.',
                ],
              ],
            },
            items: [
          ...
          {
            early: true,
            heading: 'Admin list 1 — CV search: the talent pool',
            text: 'What employers pay to search. Every row is a candidate whose flagged CV is actually in the index, so this list is for BROWSING and sanity-checking the pool — never for deciding.',
            table: {
              cols: ['Column', 'Shows'],
              rows: [
                ['Ứng viên', 'One row per candidate — not per CV. Opening it shows all their CVs (max 3).'],
                ['CV đang hiển thị', 'The flagged CV’s title · years — what CV search reads.'],
                ['Địa điểm · Số CV', 'Desired location, and how many CVs they hold.'],
                ['Hiển thị', 'The candidate’s own account switch: Discoverable / Hidden.'],
                ['Trạng thái CV', 'The one status set — Qualified · Not enough information · Can’t read · Not a CV · Removed. Only Qualified is actually indexed; the rest are listed so an operator can find them rather than wonder where a candidate went.'],
                ['Trích xuất được', 'e.g. “3 kinh nghiệm · 8 kỹ năng”. The QUALITY signal: a pool full of CVs sitting at the minimum is a thin pool, and this is where that becomes visible.'],
              ],
            },
            items: [
              'BOTH SOURCES APPEAR — a qualifying Saramin CV and a passing uploaded PDF are equally in the pool. Nothing here reveals which route a candidate took, because nothing downstream cares.',
              'NO AGE COLUMN — nothing in this list is waiting on anyone.',
            ],
          },
          {
            early: true,
            heading: 'Admin list 2 — CV check: the failures',
            text: 'A queue to work down, not a pool to browse. Only uploaded PDFs that failed the rule; a Saramin CV never appears, because the rule is arithmetic over typed fields and the fix belongs to the candidate.',
            table: {
              cols: ['Column', 'Shows'],
              rows: [
                ['Ứng viên · File', 'Who, and which file — the file is the thing being judged.'],
                ['Trạng thái CV', 'Only the two that need attention: Not enough information (a verdict is owed) and Can’t read (a nudge is owed).'],
                ['Trích xuất được', 'What we actually got — “0 kinh nghiệm · 1 kỹ năng”. The whole basis of the call.'],
                ['Đơn ứng tuyển đang chờ', 'How many applications this ONE decision releases, with the time left on the 24h net.'],
                ['Chờ đã (age)', 'Load-bearing: nothing releases a held CV automatically, so an ageing row is the only thing that says “work me”.'],
                ['Quyết định', 'Đúng là CV / Không phải CV — on “chưa đủ thông tin” rows only.'],
              ],
            },
            items: [
              'ON “CHƯA ĐỦ THÔNG TIN” the call is almost always one of two things: it clearly IS a CV and our parser failed on the layout (approve, and log it as a parser gap), or it is not a CV at all (reject). Telling those apart is the whole job.',
              'ON “KHÔNG ĐỌC ĐƯỢC” there is NO verdict — nothing to approve. The applications already went out, and the only action is nudging the candidate to upload a text-based PDF.',
              'ONE DECISION, MANY APPLICATIONS — approving releases every application waiting on that CV and makes the CV searchable; rejecting drops them and recalls any already sent.',
            ],
          },
          {
            early: true,
            heading: 'Every case',
            table: {
              cols: ['Case', 'Apply', 'CV search'],
              rows: [
                ['Saramin CV passes', '✅', '✅'],
                ['Saramin CV fails', '❌ greyed', '❌ toggle disabled'],
                ['Uploaded PDF passes', '✅ sent', '✅'],
                ['Uploaded PDF — không đọc được nội dung (ảnh scan)', '✅ sent, no wait', '❌ until a text-based file is uploaded'],
                ['Uploaded PDF — chưa đủ thông tin', '✅ applied, ⏳ delivery waits', '❌'],
                ['…admin approves the CV', '✅ all waiting applications send', '✅ becomes searchable'],
                ['…admin rejects the CV', '❌ waiting dropped, sent ones recalled', '❌'],
                ['…nobody reviews within 24h', '✅ auto-sent', '❌ CV status unchanged'],
                ['Candidate adds the missing skills by hand', '✅', '✅ no review needed'],
                ['Candidate replaces the file', 'resets — re-parsed and re-evaluated', '❌ until it passes'],
                ['A flagged Saramin CV drops below the rule after an edit', '❌ greyed, immediately', '❌ keeps the flag but leaves the index — see below'],
              ],
            },
          },
          {
        ...
        behaviors: [
          ...
          'A “không đọc được nội dung” CV is sent to employers normally — a human can read the file even when we cannot — and the candidate is told only that it will not appear in CV search until they upload a text-based PDF.',
        ...
        states: [
          ...
          'Chưa đủ thông tin — the CV is unsearchable AND its applications wait for the verdict',
          'Không đọc được nội dung — the CV is unsearchable, but applications are sent normally',
