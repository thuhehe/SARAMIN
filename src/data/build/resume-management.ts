  ...
  requirements: [
    ...
    },
    {
      ...
      table: {
        ...
        rows: [
          ['Skills', '20', '**4 or more** of the job’s skills are on the CV. The rule is one line: **5 points per matched skill, capped at 20.**', 'Fewer than 4 matched → 5 points each. Two matched → 10.', 'No overlap at all — or the CV yielded no skills we could read, in which case the block is DROPPED and renormalised rather than scored zero.'],
          ['Years of experience + career level', '22', 'Candidate years fall inside the job’s stated range and the level agrees.', 'Under the minimum → 20 × (years ÷ min). Over the maximum → 20 − 2 per year over, floor 12; over-qualified is still a real candidate.', 'Zero years against a role with a stated minimum.'],
          ['Location + work type', '21', 'The job is remote and the candidate accepts remote (city is then irrelevant) — OR a desired location contains the job’s location and the candidate accepts the job’s work type.', '15 for an adjacent province in the same region · 12 for a city miss where the candidate will relocate · 7 for a city hit where the work type is wrong (remote-only candidate, in-office job).', 'No location overlap, no relocation, and no work-type overlap.'],
          ['Desired job role + category', '18', 'The job’s `job_role` IS the candidate’s desired role — an exact id match, both sides having resolved to the same master row.', '9 when the CATEGORY matches but the role does not · 4 for a sibling category in the same group.', 'Unrelated category.'],
      ...
      items: [
        ...
        'SO: WHEN A CV YIELDS NO READABLE SKILLS, THE SKILLS BLOCK IS DROPPED AND RENORMALISED — its 20 points are redistributed proportionally across the seven signals that remain, exactly as the RENORMALISE RULE already does for a job that states no requirement. The candidate is then ranked on what we CAN read (years, level, location, category, salary, industry, education, language) instead of being pushed to the bottom for a document we could not open. It needs no new data and no better taxonomy, which is why it ships first.',
        'THE ONE NUMBER THAT DECIDES WHETHER 20 IS STILL RIGHT — track the share of CVs that yield ≥ 3 readable skills, split by route (typed vs uploaded). This weight was cut from 38 to 20 on exactly this kind of measurement; it moves again, in either direction, on the same evidence. Skills no longer carry the largest weight in the model — Years + level (22) and Location + work type (21) do — and that ordering is itself a statement about which data we currently trust.',
