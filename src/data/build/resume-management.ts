...
export const resumeManagement: BuildModule = {
  ...
  requirements: [
    ...
    },
    {
      ...
      table: {
        ...
        rows: [
          ...
          ['2 · Desired work condition', 'Profile', '1 per jobseeker', 'Desired job role · desired job category · desired industry · desired work location · expected salary — all five, per the client field sheet', 'ONBOARDING — the wizard that runs straight after sign-up, and editable afterwards'],
      ...
      items: [
        ...
        'Upload → convert: an uploaded PDF is parsed into the CV template and the gaps are flagged for the candidate to fill — but NOTHING here gates applying. An uploaded file is always selectable at apply; only a Saramin CV carries a send gate (see APPLY-ELIGIBLE), and a doubtful file is checked AFTER submit, on the application (see Application management → the “is this a CV?” check). Uploading a file to attach to ONE application does not touch the searchable CV.',
    ...
    },
    {
      ...
      items: [
        'NO ACCEPTANCE GATE ON CONTENT — superseded. An earlier draft refused a file whose parse found no work experience and no education; that rule is GONE. The site never blocks an upload or an apply: whatever the candidate uploads is saved as a CV and is usable immediately. A blank page, an image-only scan or an unrelated document is caught AFTER submit, where it holds that one APPLICATION as Pending for a human — never the upload, never the account (Application management → “is this a CV?”, blocks 4–4e). Only TYPE and SIZE are still checked at the picker, because those are facts about the file rather than a judgement of its contents.',
    ...
    {
      label: 'SEARCH QUALITY TRACKING — every zero-result search is classified, from day one',
      text: 'Not a feature and not a phase-2 nicety: it ships WITH the matching logic, because until it exists nobody can tell “the search worked and the pool is empty” from “the search is broken”. Every CV search returning ZERO results is stamped, at query time, with exactly one of two causes.',
      table: {
        cols: ['', '1 · Thiếu ứng viên — supply gap', '2 · Logic chưa đúng — search defect'],
        rows: [
          ['What happened', 'The logic ran CORRECTLY. Terms resolved, filters applied, and no candidate in the pool matches.', 'The system SHOULD have returned results and did not.'],
          ['Is it a bug?', 'No. This is market information.', 'Yes. This is ours to fix.'],
          ['Owner', 'Sales / sourcing', 'Dev + taxonomy owner'],
          ['Target', 'Never reaches zero, and is not supposed to.', 'TRENDS TO ZERO. This is the search-quality metric.'],
        ],
      },
      items: [
        'Mixing the two into one “failed searches” number is the mistake this block exists to prevent — it produces a metric nobody can act on, because half of it is not a defect.',
        'Full specification, including how the classifier decides, what gets logged, and the acceptance criteria: see the feature “Search quality tracking” in this module.',
      ],
    },
    {
