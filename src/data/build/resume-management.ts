  ...
  requirements: [
    ...
    },
    {
      ...
      items: [
        ...
        'SHOW in the locked row, because the employer must still be able to judge fit: job role / headline, gender · age, years of experience, EMPLOYER TYPE + city (“Bệnh viện tư nhân quốc tế · TP.HCM”), job title and period, the role description, SCHOOL TYPE + degree + graduation year, skills, desired location, salary expectation and availability. A recruiter screens on the shape of the career, not on the brand name of the hospital.',
    ...
    //    Authored as a research/discovery brief because we have no data model yet.
    {
      ...
      detail: {
        ...
        uiFields: [
          ...
          {
            group: 'Filter rail · 2. WORK PREFERENCE — the candidate’s own asks',
            items: [
              { name: 'expectedSalary', type: 'currency switch + from/to dropdowns', notes: 'reads `expectedSalary { kind, currency, min, max }` — the field ALREADY carries currency: VND · USD, so the two-currency control needs no new data. The switch re-labels the bounds and scopes the query to that currency; it must NOT convert, because we hold no exchange rate and a converted ask is a number the candidate never said' },
              { name: 'salary period', type: 'derived comparison rule', required: true, notes: '`kind` is ANNUAL · MONTHLY · INTERVIEW · INTERNAL_RULE. A range query MUST normalise ANNUAL to monthly (÷12) WITHIN the same currency before comparing, or a 30,000 USD/year ask is matched against a 30,000/month bound. Division by 12 is arithmetic, not an exchange rate — it is safe; currency conversion is not' },
              { name: 'negotiable asks', type: 'behaviour, no longer a checkbox', required: true, notes: 'the "Also include Thỏa thuận" checkbox was CUT from the UI (2026-08-12). The rule it protected still holds and now has to live in the query instead: `kind = INTERVIEW` and `kind = INTERNAL_RULE` are VALUES, not nulls, and a salary range must INCLUDE them by default rather than silently dropping every candidate open to negotiation. If the client wants them excluded, that becomes an explicit choice somewhere — it must not be an accident of the range query' },
              { name: 'desiredJobCategories', type: 'multi-select dropdown (derived options)', notes: 'labelled "Job category" — the same category master the job form writes, so both sides of a match share one vocabulary' },
              { name: 'targetIndustries', type: 'multi-select dropdown (derived options)', notes: 'labelled "Desired industry" — the industry they WANT. Distinct from "Industry experience" in Work information, which is where they have actually worked; both fields exist and both are useful' },
              { name: 'desiredLocations', type: 'multi-select dropdown (derived options)', notes: 'labelled "Wants to work in" — the one a recruiter usually MEANS when they filter by location' },
              { name: 'desiredEmploymentTypes', type: 'multi-select dropdown', notes: 'FULL_TIME · CONTRACT · FREELANCE · INTERN · DISPATCH · ENTRUSTED · PART_TIME — the existing enum, not a new one. Show only the values present in the pool' },
              { name: 'availability / noticePeriod', type: 'dropdown', notes: 'Open now · Within 1 month · 2+ months' },
              { name: 'desiredWorkTypes', type: 'CUT from the rail', notes: 'in-office · remote · hybrid · oversea was removed on client direction (2026-08-12). The field still exists on the profile; re-adding the filter is a one-line change' },
              ...
              { name: 'AVAILABLE, not filtered', type: 'note', notes: '`careerLevel` (FRESHER · EXPERIENCED · ANY) and `willingToRelocate` exist on the same Job-preferences group and could be facets. Left out — careerLevel overlaps Years of work experience. Add on request; no data work needed' },
            ],
          },
          {
            group: 'Filter rail · 3. PROFILE — basic information + the two summary attributes',
            items: [
              { name: 'gender', type: 'dropdown', notes: 'Any · Nam · Nữ. Legal sign-off required before launch (see key points)' },
              { name: 'age', type: 'dropdown', notes: 'Under 25 · 25–34 · 35–44 · 45+ — BUCKETS, never a free min/max number pair, so the UI cannot be pointed at a single birth year' },
              { name: 'nationality', type: 'dropdown (derived options)', notes: 'Basic information. Legitimate for roles with a work-permit or language requirement; also a protected characteristic — see the warning on this group' },
              { name: 'maritalStatus', type: 'dropdown', notes: 'Basic information. See the warning on this group: of every filter on this screen, this is the one with no defensible hiring purpose' },
              { name: 'currentLocation', type: 'multi-select dropdown (derived options)', required: true, notes: 'labelled "Currently living in" — the province the candidate LIVES in. Distinct from where they want to work; see the location split' },
              { name: 'highestEducationLevel', type: 'dropdown', notes: 'High school · College · Bachelor · Master · Doctor. MOVED here from Work information: the highest level reached is a summary attribute a recruiter screens on, while the individual education entries stay CV content' },
              { name: 'graduationStatus', type: 'dropdown', notes: 'Graduated · Expected graduate — the campus-hiring filter, kept beside Highest education because they are read together' },
              { name: 'totalYearsExperience', type: 'dropdown', notes: 'No experience / fresher · 1–3 · 3–5 · 5+ years. MOVED here for the same reason as education: the total is a summary attribute, the individual jobs are CV content. The fresher bucket is what makes campus hiring possible' },
            ],
          },
          {
            group: 'Filter rail · 4. WORK INFORMATION — CV content',
            items: [
              { name: 'jobTitle', type: 'multi-select dropdown (derived options)', required: true, notes: 'resolves to the Title taxonomy. Its own field, not only reachable through the keyword box — title is the first thing a recruiter narrows on' },
              { name: 'industry (experience)', type: 'multi-select dropdown (derived options)', notes: 'labelled "Industry experience" — where they HAVE worked, as against "Desired industry" in Work preference' },
              { name: 'skills', type: 'multi-select dropdown (derived options)', required: true, notes: 'options come from the skills present in the CURRENT results, top N by count. Autocompletes against the Skill taxonomy — ids, never strings' },
              { name: 'certification', type: 'multi-select dropdown (derived options)', notes: 'from CandidateProfile.certifications. In regulated sectors (healthcare, accounting) this is often the hardest requirement of the whole search' },
              { name: 'language + level', type: 'ONE paired control', required: true, notes: 'rendered as a single bordered block containing two dropdowns, because it is one question ("English, at professional level") and two loose dropdowns invite the wrong query. They MUST be applied together against the SAME languages[] entry: English-professional must not match a candidate who is basic in English and professional in Korean' },
            ],
          },
          {
        ...
        sections: [
          ...
          },
          {
            ...
            table: {
              ...
              rows: [
                ['1 · CV ACTIVITY', 'Last resume update', 'Not candidate data — our own record of when the CV was last touched. FIRST in the rail because freshness is the cheapest way to avoid spending a credit on someone who has stopped looking.'],
                ['2 · WORK PREFERENCE', 'Desired salary (VND/USD) · Job category · Desired industry · Wants to work in · Employment type · Availability', 'The candidate’s own asks, captured at onboarding. ALL OPTIONAL, and a CV can never supply them — it records where someone HAS BEEN, never where they want to go. Filtering here narrows hard: it drops every CV that left the field blank as well as every CV that does not match.'],
                ['3 · PROFILE', 'Gender · Age · Nationality · Marital status · Currently living in · Highest education · Graduation · Years of work experience', 'Basic information, plus the two SUMMARY attributes a recruiter screens on before reading any detail. Highest education and total years live here; the individual schools and jobs stay in Work information.'],
                ['4 · WORK INFORMATION', 'Job title · Industry experience · Skills · Certification · Language + level', 'CV content — the substance of the search. Complete once the parser is live; THIN in a Phase-1 launch without AI, where these come only from the light review step.'],
              ],
            },
            items: [
          ...
          {
            heading: 'Nationality and marital status as filters — read this before building them',
            text: 'Both are now filters, on client direction (2026-08-12). Both are in the data already, so there is no technical objection to make. The objection is a legal one, and it is not the same objection for the two fields.',
            table: {
              cols: ['Field', 'Defensible hiring use', 'Exposure'],
              rows: [
                ['Nationality', 'Real, and common in Vietnam: work-permit eligibility, a role requiring a native speaker, a role at a foreign-invested employer with a nationality quota.', 'A protected characteristic in most jurisdictions. Defensible where the requirement is genuine and recorded; indefensible as a general screen.'],
                ['Marital status', 'None that we can identify. It does not predict performance, availability, or tenure.', 'The single most challengeable field on this screen. In practice it is used to infer whether a woman is likely to take maternity leave, which is exactly the inference discrimination law exists to prevent — and our query log would record us providing the tool.'],
                ['Gender · age', 'Effectively none as a filter, though both are legitimate to DISPLAY on a row.', 'Already flagged in the key points; the same legal sign-off covers all four fields.'],
              ],
            },
            warn: 'RECOMMENDATION, unchanged by the fields being available: ship nationality if the client wants it, and put MARITAL STATUS in front of their legal counsel before it goes live. If it stays, log every query that uses it (FacetAudit already does) and be able to say why. Cutting one dropdown is a five-minute change; defending a discrimination claim is not.',
          },
          {
          ...
          },
          {
            ...
            table: {
              ...
              rows: [
                ...
                ['WORK PREF — Desired salary (VND · USD)', 'expectedSalary { kind, currency, min, max }', 'Job preferences — currency is ALREADY on the field; no data work needed'],
                ['WORK PREF — Job category', 'desiredJobCategories[]', 'Job preferences — the same category master the job form writes'],
                ['WORK PREF — Desired industry', 'targetIndustries[]', 'Job preferences'],
                ['WORK PREF — Wants to work in', 'desiredLocations[]', 'Job preferences (onboarding)'],
                ['WORK PREF — Employment type', 'desiredEmploymentTypes[]', 'Job preferences — FULL_TIME · CONTRACT · FREELANCE · INTERN · DISPATCH · ENTRUSTED · PART_TIME'],
                ['WORK PREF — Availability', 'availability / noticePeriod', 'Candidate asks'],
                ['PROFILE — Gender', 'gender', 'Basic information — optional, reinstated 2026-08-09'],
                ['PROFILE — Age', 'dateOfBirth', 'Basic information — age is COMPUTED at query time, never stored'],
                ['PROFILE — Nationality', 'nationality', 'Basic information — see the legal note on this group'],
                ['PROFILE — Marital status', 'maritalStatus', 'Basic information — see the legal note on this group'],
                ['PROFILE — Currently living in', 'location (province/city)', 'Basic information / extracted from the CV'],
                ['PROFILE — Highest education', 'education[].degree → level enum', 'DERIVED — the highest level across all education entries'],
                ['PROFILE — Graduation', 'education[].endDate', 'DERIVED — an end date in the future is "expected graduate"'],
                ['PROFILE — Years of work experience', 'yearsOfExp / totalYearsExperience', 'DERIVED from work history, candidate-overridable'],
                ['WORK INFO — Job title', 'headline / currentTitle', 'Extracted from the CV → Title taxonomy'],
                ['WORK INFO — Industry experience', 'industry', 'Extracted from the CV → Industry taxonomy'],
                ['WORK INFO — Skills', 'CvSkill[] (cvId · skillId · source)', 'Extracted or hand-added → Skill taxonomy'],
                ['WORK INFO — Certification', 'certifications[] { name, issuer, date }', 'Extracted from the CV'],
                ['WORK INFO — Language + level', 'languages[] { language, proficiency }', 'The `language` (8 rows) and `language_proficiency` (7 rows, CEFR) masters. Indexed AS A PAIR — languages are NOT skills'],
              ],
            },
            warn: 'CUT on client direction (2026-08-12), all four backed by real fields and all re-addable in minutes if wanted back: desiredWorkTypes (in-office · remote · hybrid · oversea), the "include Thỏa thuận" checkbox, and the two credit-protection toggles (exclude already-unlocked, exclude already-in-pipeline). Also recorded so nobody builds from an earlier draft: "job type" was never a new field (it is `desiredEmploymentTypes`), and `expectedSalary` always carried currency VND · USD — the older `desiredSalary: int (VND)` sketch in CV data & matching architecture is superseded by it.',
          },
          {
            heading: 'Salary — two currencies, two periods, no conversion',
            text: 'The pool holds asks in VND and in USD, monthly and annual. `expectedSalary { kind, currency, min, max }` already carries all of that; the filter has to respect it rather than flatten it.',
            items: [
              'PERIOD: `kind` is ANNUAL · MONTHLY · INTERVIEW · INTERNAL_RULE. A range query must normalise ANNUAL to monthly (÷12) WITHIN the same currency before comparing, or a 30,000 USD/year ask matches a 30,000/month bound. Dividing by 12 is arithmetic and always safe; converting between currencies is not.',
              'CURRENCY: the switch re-labels the bounds and scopes the query to that currency. It must NOT convert — we hold no exchange rate, any rate is stale within weeks, and a converted salary shown to an employer is a number the candidate never said. A candidate who asked in USD is simply not in the result set of a VND-bounded query, and the panel should not pretend otherwise.',
              'Bounds are PRESETS in a dropdown, not free number entry. Free entry invites 15000000 against 15 and a result set of zero.',
              'INTERVIEW ("thỏa thuận") and INTERNAL_RULE ("theo quy định công ty") are VALUES, not nulls, and share one checkbox under the range. Without it, setting any bound silently drops every candidate open to negotiation.',
        ...
        backend: {
          ...
          dataModel: [
            ...
            { name: 'expectedSalary', type: '{ kind, currency, min?, max? }', notes: 'EXISTING FIELD from Job preferences — kind: ANNUAL · MONTHLY · INTERVIEW · INTERNAL_RULE, currency: VND · USD. Index a derived monthly figure PER CURRENCY for range comparison, and keep the written value for display. Never converted between currencies' },
        ...
        openQuestions: [
          ...
          'CLOSED — the two-currency salary filter needs no new field either. `expectedSalary` already carries `currency: VND · USD` and `kind: ANNUAL · MONTHLY · INTERVIEW · INTERNAL_RULE`. What DOES need confirming is the comparison rule: we normalise ANNUAL to monthly (÷12) within one currency and never convert across currencies, so a USD ask never appears in a VND-bounded search. Confirm that is acceptable rather than showing an approximate conversion.',
