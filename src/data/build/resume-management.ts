        ...
        sections: [
          ...
          },
          {
            ...
            items: [
              ...
              '★ ON A LIST OF CVs, ASK THE CANDIDATE’S CHOICE FIRST (decided 2026-08-23). The admin columns “Application status” and “CV Search status” were RENAMED to **Dùng để ứng tuyển** and **Dùng cho CV search**, because as statuses they collapsed two unrelated facts into one word: a CV nobody ever applied with read “Not sent”, and so did a CV whose applications we are holding. Identical value, opposite situations, and only the second is the reviewer’s business.',
              'A CV NOBODY APPLIED WITH HAS NO APPLICATION STATUS, it has an ABSENCE — printing “Not sent” for a record that does not exist is how a list starts lying. **Dùng để ứng tuyển** therefore answers only what happened: “✓ 2 đơn ứng tuyển” or “— Chưa dùng để ứng tuyển”. What is happening TO those applications is not restated, because the CV status column sits on the same row and already implies it.',
              'ONE LINE PER CELL (2026-08-23). A first version printed a second line spelling out the consequence — “2 đơn đang giữ lại — chờ duyệt · 5h”. It was cut: on a twelve-column table it cost double row height to repeat what the neighbouring status column says, which is the trade that made the redundancy obvious. A derived cell earns its place by answering ONE question and stopping.',
              'THE BLANK IS THE USEFUL PART. A queue row reading “— Chưa dùng để ứng tuyển” is one where the decision releases NOTHING and nobody is waiting — visible at a glance, and completely hidden by the old “Not sent”. It is the fastest way to triage a backlog: rows with applications waiting are the ones costing someone something.',
              'SEARCH VISIBILITY IS NOT DERIVABLE FROM CV STATUS ALONE, and Talent pool had this wrong: it computed the cell from status only, so a Qualified CV the candidate never flagged rendered as “Showing”. A jobseeker flags ONE of their up-to-three CVs, so `searchPick` is the candidate’s own toggle and we never write it. Visibility is `searchPick` **AND** Qualified — two inputs, and “chưa chọn” is the common, healthy case rather than a defect to fix.',
              'THE APPLICANTS SCREEN KEEPS “Application status” UNCHANGED. A row there IS an application, so it genuinely has a status. The rename applies only where a row is a CV.',
              '★ THE TWO SCREENS ASK DIFFERENT HALVES OF THE SAME FIELD, and the split is not cosmetic. On **CV review** the column is **Vào Talent pool** — ✓ Đã chọn · — Chưa chọn — because the only fact a reviewer cannot work out for themselves is the CANDIDATE’S CHOICE; whether the CV is visible follows from the CV status in the next column (anything in doubt is hidden, approve a picked CV and it shows). On **Talent pool** the choice is constant — everyone there switched CV search on — so that screen asks the only question left: **CV Search status**, Showing · Hidden.',
              'NAME A COLUMN FOR THE QUESTION IT ANSWERS, not for the field behind it. Both columns read `searchPick`, and calling both “CV Search status” would have made the queue’s cell look like a visibility report it is not. One shared column set, one overridden label.',
              'VISIBILITY NEEDS BOTH INPUTS — the candidate’s own toggle AND Qualified. Deriving it from CV status alone was the original bug: a Qualified CV nobody flagged rendered as Showing.',
              'BOTH COLUMNS ARE BINARY, which is the point — an earlier draft gave CV review three values (Showing · Hidden · Chưa chọn) and it read as two questions crammed into one cell. Splitting by screen gives each a clean pair and removes the inference the reader was being asked to make in the wrong place.',
              '“— CHƯA CHỌN” IS THE USEFUL VALUE ON THE QUEUE. It tells the reviewer this decision changes nothing about search: the candidate never offered this CV to the pool. Together with “— Chưa dùng để ứng tuyển” it marks the rows where approving or rejecting releases nothing at all — the fastest triage the queue gives.',
              'TALENT POOL DOES NOT CARRY “Dùng để ứng tuyển”. That screen lists CVs, not applications, and an application count is the Applicants screen’s business.',
              '★ ON A QUEUE YOU WORK THROUGH TABS; ON A CATALOGUE YOU FILTER (decided 2026-08-23). Talent pool’s tab row used to read All · Approved · Pending review · Rejected · Not enough information · Can’t read — four REVIEW-WORKFLOW states on a screen that cannot work them. That was the real duplication with CV review, and it is why the two screens reported different pending counts (10 vs 13). The tabs are gone; CV review keeps its own, because it IS the job.',
              'A TAB IS A PROMISE THAT THIS SUBSET IS A JOB, and a catalogue of 8,420 rows that never reaches zero cannot keep it. A FILTER says “let me look”, which is what a catalogue is for. CV status stays one click away on Talent pool as an explicit filter — nothing is lost except the false implication that browsing it was work.',
              'THE FILTER HAD TO BE EXPLICIT, and the reason is worth knowing before someone “simplifies” it: the list frame can normally DERIVE a filter from any column whose values repeat, but not this one — the admin’s recorded `why` renders inside the CV-status cell, so the cell text is unique per row and the derivation correctly rejects it. Deriving filters from rendered text is right for most lists and wrong here, so the screen supplies its own and filters on the FIELD.',
              'THE BADGE IS WHAT REPLACES THE TABS. `CV review ⑩` in the admin nav, amber, hidden at zero — because a queue nobody knows has work in it is the same as no queue at all, and an empty queue should look like nothing to do rather than like a page reporting in. Badges go ONLY on pages that are queues; a number beside a catalogue is noise, since a catalogue is never “done”.',
              'THE MERGE WAS CONSIDERED AND REJECTED. Combining CV review into Talent pool was proposed on the grounds that both are “simply the CV list” — true of the ROWS, false of the JOB. Talent pool is read-mostly reference you go to when someone asks about a candidate; CV review is write-mostly work you go to at the start of a shift and leave when it is empty. Same records, different verb, the way an email Inbox and All Mail are the same messages. Demoting the daily job into a tab on the occasional one gets the hierarchy backwards; the duplication was real but it was pointing the other way.',
