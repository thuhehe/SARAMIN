      ...
      detail: {
        ...
        uiFields: [
          ...
          },
          {
            ...
            items: [
              ...
              { name: 'range semantics', type: 'query rule', required: true, notes: 'OVERLAP, not containment: `candidate.min ≤ filter.max AND candidate.max ≥ filter.min`. A missing candidate max is +∞, a missing candidate min is 0. Getting this backwards is the most common way this filter is built wrong, and it looks like an empty pool rather than a bug' },
        ...
        sections: [
          ...
          },
          {
            ...
            text: 'The pool holds asks in VND and in USD, monthly and annual, and a large share holds no number at all. `expectedSalary { kind, currency, min?, max? }` already carries all of that; the filter has to respect it rather than flatten it. The rules below are ordered by how easy they are to get wrong.',
            table: {
              cols: ['What the candidate stated', 'Employer filters VND 15 – 25 triệu / month', 'Why'],
              rows: [
                ['MONTHLY · VND · 18 – 22 tr', 'MATCH', 'Same currency, the two ranges OVERLAP.'],
                ['MONTHLY · VND · 24 – 30 tr', 'MATCH', 'Overlap, not containment — 24 falls inside the filter. A candidate whose range merely touches the employer’s is still worth a call.'],
                ['MONTHLY · VND · 30 – 40 tr', 'no match', 'No overlap at any point.'],
                ['MONTHLY · VND · min 12, no max', 'MATCH', 'An open top is treated as +∞: "from 12 triệu" overlaps 15 – 25.'],
                ['MONTHLY · VND · max 16, no min', 'MATCH', 'An open bottom is treated as 0.'],
                ['ANNUAL · VND · 240 – 300 tr / year', 'MATCH', 'Normalised ÷12 to 20 – 25 tr / month FIRST, then compared. Without this a 30,000/year ask matches a 30,000/month bound.'],
                ['MONTHLY · USD · 1,000 – 1,500', 'not shown, but COUNTED', 'Different currency: never compared numerically. Excluded from this result set and reported under it — "12 candidates ask in USD — switch the currency to see them."'],
                ['kind = INTERVIEW ("thỏa thuận")', 'MATCH', 'Negotiable is a VALUE, not a null. There is no number to fail the comparison, and these are often the candidates most worth calling.'],
            ...
            items: [
              'OVERLAP, NOT CONTAINMENT — the rule the naive implementation gets wrong. Match when `candidate.min ≤ filter.max AND candidate.max ≥ filter.min`. Writing `candidate.min ≥ filter.min AND candidate.max ≤ filter.max` instead requires the candidate’s whole range to sit inside the employer’s, which throws away most of the sensible matches and looks like an empty pool.',
