    ...
    },
    {
      ...
      table: {
        ...
        rows: [
          ...
          ['4 · Suggest', '“Common for {desired role}: ＋Figma ＋Wireframing …” — one tap each.', 'Keyed on PROFILE → desired job role, read from the backend’s EXISTING `occupation_skill` map (occupation ↔ skill, each row flagged ESSENTIAL or OPTIONAL). Order: ESSENTIAL first, then OPTIONAL, then taxonomy order. Excludes what is already on the CV. Cap ~6 chips.'],
      ...
      items: [
        ...
        '3 · Suggestions come from the DESIRED ROLE — the editor shows "Common for {desired role}: ＋Figma ＋Wireframing …", one tap each, read from `occupation_skill`. See stage 4 in the table above for the full ordering rule.',
    ...
    },
    {
      ...
      table: {
        ...
        rows: [
          ...
          ['3 · Resolve', 'One exact lookup against an index built from `skill.name_vi` + `skill.name_en` + every `skill_alias.alias`, all normalised.', 'EXACT match only. No fuzzy, no edit distance, no “closest row”. An unattended import has no human to correct a wrong guess, so a wrong guess must be impossible.'],
          ['4 · Branch on the outcome', 'Hit → confirmed chip. Miss → unconfirmed chip + the term is written to `unmatched_term`. Ambiguous → treated as a MISS.', 'Ambiguity can only arise if two rows share a lookup key, which CI forbids — but if it happens, import must never pick. See the alias rules: this is why “PM” is nobody’s alias.'],
          ['5 · Candidate confirms', 'The review screen shows matched skills as normal chips and unmatched terms separately, each offering “pick from the list” or “request it”.', 'NOTHING becomes a CvSkill row until the candidate confirms. AI extraction is a suggestion engine, never an authority — this is also the answer when the client asks “what if the AI is wrong”.'],
        ],
      },
      items: [
        ...
        'This pipeline is why “extract, don’t ask” is safe to promise. The candidate confirms a short list of chips instead of typing them; the cost of a bad extraction is one rejected chip, not corrupted data.',
      ],
    },
    {
      ...
      table: {
        ...
        rows: [
          ...
          ['`skill_alias` / `industry_alias` / `job_role_alias`', 'Exactly our lookup keys. Their own doc calls it “node aliases (CV/typeahead normalization)”, with a `lower(alias)` index.', '**0**', '🟡 BLOCKER — schema built, never populated.'],
          ...
          ['`unmatched_term`', 'Admin approve / merge / reject queue — the miss path in the import pipeline above.', '—', '🟡 Exists, unused.'],
      ...
      items: [
        ...
        'CLEANUP MUST PRECEDE ALIAS SEEDING. Writing aliases onto a broken list entrenches the breakage — map “pm” onto the row literally named `Project Manager` and the mistake is permanent.',
        ...
        'WHAT TO ASK FOR, in order: (1) a cleanup pass — retire job titles and languages, fix typos, merge spelling variants INTO `skill_alias` rather than deleting them, group the 141; (2) grow the non-IT groups to usable depth; (3) seed `skill_alias` with the diacritic-free VN spellings and the EN/VN pairs; (4) grow `occupation_skill` to ~10 rows per role. Steps 1–2 are the client’s taxonomy work; steps 3–4 are where our seed file is the worked example.',
      ],
      warn: 'Phase-1 skills UI cannot ship ahead of this. Autocomplete against 731 dirty rows with no aliases means a candidate typing “ke toan”, “JS” or “CSKH” finds nothing, and the fallback — “request «x»” — routes to a curation queue nobody is staffed to work. Agree an owner and a date for the cleanup before the skills screens are estimated.',
    },
    {
