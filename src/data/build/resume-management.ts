  ...
  requirements: [
    ...
    },
    {
      ...
      items: [
        ...
        'THE PROTECTED RULE IS SYMMETRIC — gender, marital status and age NEVER enter the score, in either direction. They were removed as employer filters on 2026-08-13 for exactly this reason, and using them to rank a candidate’s recommendations would be the same discrimination wearing the other hat. Recommending by age is age-based targeting whichever end initiates it.',
      ...
      detail: {
        ...
        sections: [
          ...
          },
          {
            ...
            text: 'The reviewer picks a reason code; the code picks the message, including the chip — “Chưa được duyệt — <reason>” (softened from “Bị từ chối”, per-reason so the three fixes are tellable apart at a glance; both on client feedback). Three places must agree: the chip, the ONE-LINE summary on My CVs, and the full explanation on CV detail — the page a candidate opens precisely to find out what is wrong. So they are written here together.',
            table: {
              cols: ['Reject reason (admin picks)', 'Chip on My CVs', 'My CVs — one line + one button', 'CV detail — heading + body'],
              rows: [
                [
                  'Can’t read',
                  'Chưa được duyệt — Không đọc được (rose)',
                  '“File dạng ảnh scan nên hệ thống không đọc được nội dung.” → Tải lên CV khác',
                  '“Hệ thống chưa đọc được hồ sơ của bạn” — “CV của bạn hợp lệ, nhưng file là bản scan/ảnh nên chúng tôi không trích xuất được nội dung. Hãy tải lên bản PDF dạng văn bản.” NEVER blame the candidate here: the file is unreadable to US.',
                ],
                [
                  'Not a CV',
                  'Chưa được duyệt — Không phải CV (rose)',
                  '“File bạn tải lên không phải một CV.” → Tải lên CV khác',
                  '“Hồ sơ chưa được duyệt” — “File bạn tải lên không phải một CV (giấy tờ, bảng giá, ảnh không liên quan). Hãy tải lên CV của bạn, hoặc tạo Saramin CV.”',
                ],
                [
                  'CV but not enough information',
                  'Chưa được duyệt — Thiếu thông tin (rose)',
                  '“Hồ sơ chưa đủ thông tin để gửi tới NTD.” → Cập nhật hồ sơ',
                  '“Hồ sơ chưa đủ điều kiện” + the missing fields as a checklist, each with its own hint (“Chưa có kinh nghiệm? Điền Học vấn + Dự án thay thế”). This is the one rejection with a self-service fix, and it is the commonest.',
                ],
                [
                  '— (no rejection; the scan alone)',
                  '⚠ Chưa đủ thông tin (amber)',
            ...
            items: [
              'THREE MESSAGES FOR AN UPLOAD, ONE FOR A SARAMIN CV. A typed CV can only be “Chưa đủ thông tin”, so the other two messages must never be reachable from it — and its fix is always **Cập nhật hồ sơ**, never **Tải lên CV khác**. (The on-screen legend that used to state this on My CVs was REMOVED, 2026-08-20 review — a status legend is spec material; the screen teaches through the rows themselves.)',
              ...
              'ALL THREE REASONS HAVE A SELF-SERVICE FIX now that abuse is no longer a CV reason — upload a different file, or fill the missing fields. Abuse routes to **Block user** instead, which is an account action with its own messaging and no self-service path.',
          ...
          {
            early: true,
            heading: 'THE TAG MATRIX — every reject reason on every surface that shows it',
            text: 'One rejection is visible in THREE places: the CV on My CVs, its applications on My applications, and — when one had already been delivered — the employer’s Applicants list. The chip always states the STATUS; the reason rides in the line or banner under it, never as a chip of its own. Written as one table so the three surfaces can never drift apart.',
            table: {
              cols: ['Reject reason', 'Jobseeker — My CVs (CV tag)', 'Jobseeker — My applications (application tag)', 'Employer — Applicants (application tag)'],
              rows: [
                [
                  '**Not a CV**',
                  'Rose chip **Chưa được duyệt — Không phải CV** · line: “File bạn tải lên không phải một CV. Hãy tải lên CV của bạn, hoặc tạo Saramin CV.” → **Tải lên CV khác**',
                  'Never delivered → rose **Không được gửi** · “File này không phải một CV — tải lên CV khác rồi ứng tuyển lại.” · Was delivered → rose **Đã thu hồi** · “Saramin đã thu hồi CV này — file không phải một CV.”',
                  'Row chip **Đã thu hồi** (rose), name struck through, no match score. Detail banner: “Saramin đã thu hồi CV này. Lý do: File này không phải một CV.” View/Download removed; an unlock is auto-refunded.',
                ],
                [
                  '**CV but not enough information**',
                  'Rose chip **Chưa được duyệt — Thiếu thông tin** · line: “Hồ sơ chưa đủ thông tin để gửi tới nhà tuyển dụng.” → **Cập nhật hồ sơ**',
                  'Never delivered → rose **Không được gửi** · “Hồ sơ chưa đủ thông tin — cập nhật hồ sơ rồi ứng tuyển lại.” · Was delivered → rose **Đã thu hồi** · “Saramin đã thu hồi CV này — hồ sơ chưa đủ thông tin.”',
                  'Same chip and treatment. Detail banner reason: “CV chưa đủ thông tin.”',
                ],
                [
                  '**Can’t read**',
                  'Rose chip **Chưa được duyệt — Không đọc được** · line: “File dạng ảnh scan nên hệ thống không đọc được nội dung. Hãy tải lên bản PDF dạng văn bản.” → **Tải lên CV khác**',
                  'Never delivered → rose **Không được gửi** · “File là bản scan nên hệ thống không đọc được — tải bản PDF dạng văn bản rồi ứng tuyển lại.” · Was delivered → rose **Đã thu hồi** · “Saramin đã thu hồi CV này — file là bản scan nên hệ thống không đọc được.”',
                  'Same chip and treatment. Detail banner reason: “File là bản scan, hệ thống không đọc được nội dung.”',
                ],
                [
                  '— (no rejection: DOUBT on an UPLOAD — Can’t read · Not enough information. In the admin queue, invisible to the candidate — see the rule below)',
                  '**Nothing.** The row renders exactly like a healthy CV: no chip, no reason line, no button. The CV-search chip still shows if this is the CV they picked — it marks the CHOICE, not the live index',
                  '**Nothing special** — blue **Đã nộp** · “Đơn của bạn đã được nộp”, the ordinary in-progress state. Timeline: “Gửi tới nhà tuyển dụng — Đang xử lý”. The application status underneath is **Not sent**',
                  '**Nothing** — the application has not been delivered, so no employer row exists to tag',
                ],
                [
                  '— (no rejection: a SARAMIN CV below the rule — the ONE doubt the candidate does see)',
                  'Amber chip **⚠ Chưa đủ thông tin** · “Chưa hiển thị với NTD & chưa ứng tuyển được — cần bổ sung kinh nghiệm hoặc kỹ năng.” → **Cập nhật hồ sơ**',
                  '**Nothing** — a Saramin CV below the rule cannot be selected to apply at all, so no application ever exists',
                  '**Nothing** — same reason',
                ],
              ],
            },
            items: [
              'THE MY-CVS CHIP NAMES STATUS **AND** REASON — “Chưa được duyệt — <reason>” (decided 2026-08-20, client feedback; reverses a brief one-uniform-chip draft). The three rejections have three different fixes, so they must be tellable apart without opening the row. Everywhere ELSE the chip stays status-only with the reason in the line/banner: an application’s fix is always “fix the CV”, and the employer only needs to know the row is dead.',
              'DOUBT ON AN UPLOAD IS INVISIBLE TO THE CANDIDATE (decided 2026-08-20, client feedback) — not softened, not reworded: ABSENT. No chip, no line, no button, no “chờ duyệt”, no warning in the apply modal, no flag on the application. Can’t read and Not enough information are OUR uncertainty about a file WE parsed — the layout may have beaten the parser on a CV a human reads perfectly — so announcing them tells the candidate their document is defective before anyone has confirmed it, which is precisely the “never blocked by OUR mistake” promise broken in the one place it matters. The candidate only ever sees DECIDED states: Qualified (nothing to say) or Rejected (reason + fix). An admin approval turns the CV Qualified and the applications Sent silently.',
              'THE ONE DOUBT THAT IS SHOWN is a SARAMIN CV below the rule, and it is a different thing entirely: no file, no parser, no uncertainty, nothing queued for review. It is arithmetic over fields the candidate typed into our own form, so the check cannot be wrong about it and the fix is entirely theirs — shown immediately, amber, with **Cập nhật hồ sơ**. Do not generalise from it to the upload route.',
              'THE COST IS OURS, AND IT IS THE PRICE OF THE RULE — a held application is now invisible to the person waiting on it, so an unworked queue is a SILENT failure rather than a visible one. Nothing candidate-facing complains, which means the ageing alert on the CV review queue is no longer an ops nicety: it is the only alarm in the system. Staff the queue to the SLA or this decision turns into applications that quietly never arrive.',
              '“CHƯA ĐƯỢC DUYỆT”, NOT “BỊ TỪ CHỐI” (decided 2026-08-20, client feedback: too harsh) — the verdict prefix is softened. “Chưa” is also honest: a rejection is not a ban on the person — replacing the file or adding the fields returns the CV to review, so “not approved yet” describes exactly where it stands. The rose colour, the struck search access and the reason keep it unmistakably a verdict rather than a wait.',
              '“THIẾU THÔNG TIN” IN THE CHIP, NOT “CHƯA ĐỦ THÔNG TIN” — the rejected-for-thin-content chip must never read as the amber auto-flag of the same name in the same list. One word changes (“thiếu” vs “chưa đủ”), and with the colour it keeps the two states unconfusable; the full line under both still says “chưa đủ thông tin” in plain words.',
              'ONLY TWO THINGS ARE EVER FLAGGED ON A CV ROW, so the colours carry no ambiguity: AMBER is the Saramin gate the candidate clears themselves, ROSE is an admin verdict. Everything else — healthy CVs and uploads in doubt alike — renders unflagged. A rejected chip never says “tạm”.',
              'THE JOBSEEKER VERB DEPENDS ON DELIVERY — the same rejection reads **Không được gửi** when the application never left, and **Đã thu hồi** when it was pulled back. “Thu hồi” admits something reached the employer and was withdrawn; using it for a thing that never left would be false in one direction, and “không được gửi” on a thing the employer saw would be false in the other.',
              'NEVER MUTED, NEVER “NOT SELECTED” — both rejection outcomes are rose on My applications, because the one thing they must not read as is the employer declining. The employer never saw it (or had it taken away); the fix is the candidate’s, and the tone has to say “act” rather than “archived”.',
              'THE EMPLOYER LIST CHIP IS THE SAME FOR ALL THREE REASONS — a recalled row is dead however it died, and the list only needs to say “stop working this one”. The per-reason wording lives in the detail banner, where all three are safe to show because each describes the DOCUMENT, never the person. An account-level judgement (Block user) stays a reasonless withdrawal.',
              'NOBODY SEES DOUBT — not the employer, and now not the candidate either. An application on a doubt CV was never delivered, so there is no employer row to tag; and on the jobseeker side it is deliberately indistinguishable from an ordinary application in progress. The only party who sees a doubt state is the ADMIN, in the CV review queue, which is the whole point of having one.',
            ],
          },
          {
            early: true,
            ...
            table: {
              ...
              rows: [
                ...
                ['Chưa được duyệt — “không phải CV”', 'Readable, but not a CV: an ID card, a price list, unrelated paperwork.', 'ADMIN ONLY — a machine cannot judge this', 'Needs a different FILE → Tải lên CV khác'],
              ],
            },
            items: [
              ...
              'ONLY “NOT A CV” NEEDS A HUMAN — the other two are the scan’s own output. That is what keeps the review queue small, and it is why the admin action is Reject WITH A REASON rather than a bare Reject: the reason code is what selects the candidate’s message.',
              ...
              'REMOVED TWICE, and worth recording so it is not rebuilt a third time: an “Đang kiểm tra” chip telling the candidate their CV was under review. First it went because it described the older model where doubt held only search. It came back briefly as “Chờ duyệt” and went again on 2026-08-20 for a stronger reason — no candidate-facing surface mentions the review AT ALL now, because our uncertainty about our own parse is not their news to receive. Any future ticket asking for a “pending review” chip is asking to reverse a decision, not to fill a gap.',
          ...
          {
            early: true,
            heading: 'REJECT REASON CODES — three, reusing the scan’s own words',
            text: 'A rejection records a CODE as well as the internal note. The note explains one call; only a code lets thirty of them be grouped, so a pattern becomes visible instead of staying thirty anecdotes. The three codes deliberately mirror what the SCAN says, because a rejection is the reviewer ANSWERING the scan’s question after opening the file.',
            table: {
              cols: ['Code', 'What the reviewer confirmed', 'What it tells us in aggregate'],
              rows: [
                ['**Can’t read**', 'Opened it; genuinely unreadable — image-only, corrupt, or unusable quality.', 'Rising share means candidates are uploading photos of paper. An upload-time warning is cheaper than a review.'],
                ['**Not a CV**', 'Readable, but it is not a CV — a price list, a menu, a homework file, a photo.', 'Expected volume. Rising means the upload screen is unclear about what to attach.'],
                ['**CV but not enough information**', 'A real CV, readable, but too thin to send — the rule was right.', 'The healthy case. Rising alongside a rising Approve rate means the rule is set too high.'],
              ],
            },
            items: [
              ...
              'THE CODE IS INTERNAL. The candidate reads a plain reason with the fix beside it; the code exists for us to count.',
