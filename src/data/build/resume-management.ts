    ...
    },
    {
      ...
      detail: {
        ...
        sections: [
          ...
          },
          {
            ...
            table: {
              ...
              rows: [
                ...
                ['What carries the STATUS', 'The **version** — the CV shows the status of its current one', 'The **version**'],
            ...
            items: [
              '★ THE VERSION IS THE UNIT THAT CARRIES MEANING, and getting this one line right is what makes every other rule on this page answerable. Status attaches to a version. An application references a version. A review verdict lands on a version. The CV itself is just the stable name they all hang from — so “which document did the employer receive?”, “which one did the admin reject?” and “what changed since?” all have exact answers instead of depending on when you asked.',
          ...
          {
            heading: 'WHERE THE VERSION IS ACTUALLY VISIBLE — four surfaces, and none of them optional',
            ...
            table: {
              ...
              rows: [
                ['**Admin · CV review** — row', '`bản 3` beside the Upload pill, only when > 1', 'The reviewer is opening ONE version. A CV replaced three times is a different situation from a first upload, and the row is where that registers before the file opens.'],
                ['**Admin · reject dialog** — consequence block', 'Three counted lines: *N đơn đang chờ → Không được gửi* · *N đơn đã gửi (bản 3) → Thu hồi* · *N đơn đã gửi (bản cũ) → **Giữ nguyên***', '**The load-bearing one.** It replaces a single line that promised to recall everything. The green KEPT row is the rule made visible — and it is what stops an operator rejecting in the belief they are pulling back more than they are.'],
                ['**Admin · Applicants** — CV cell', '`bản 1 · đã có bản mới` when the delivered version is behind the current one', 'Explains the only row shape that otherwise looks like a bug: CV status **Rejected** sitting beside application status **Sent**.'],
                ['**Jobseeker · My CVs** — meta line', '`bản 3 (đã thay 2 lần)`', 'Teaches the model at the point of confusion: replacing a file UPDATES this CV, so nobody burns one of their three slots to fix a typo — and applications already sent keep the version they were sent.'],
              ],
            },
            items: [
              '★ THE REJECT DIALOG IS THE ACCEPTANCE TEST for the whole version rule. Reject a CV with applications on two different versions and the dialog must show a non-zero **Giữ nguyên** count; if it shows one number and the word “thu hồi”, the old blanket behaviour is still in the code no matter what the spec says.',
              'THE THREE COUNTS COME FROM ONE QUERY, not three — group this CV’s applications by (delivered?, cvVersion == current?) and the dialog’s three rows fall out. Held = not delivered · Recalled = delivered on the current version · Kept = delivered on an earlier one.',
              'DO NOT SHOW “bản 1”. A version number on a CV that has never been replaced is noise on every row to serve the rare one. Every surface above renders the version only when there IS a history — the absence of the label is itself the information.',
              'THE EMPLOYER IS NOT TOLD THE CANDIDATE’S CV GOT WORSE, and the `bản 1 · đã có bản mới` label is internal to the admin console for that reason. What an employer may eventually be offered is that a NEWER version exists — an invitation, priced or not — never a verdict on a document they are holding and are entitled to keep.',
