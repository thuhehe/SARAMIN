      ...
      detail: {
        ...
        sections: [
          ...
          },
          {
            ...
            items: [
              'THREE MESSAGES FOR AN UPLOAD, ONE FOR A SARAMIN CV. A typed CV can only be “Chưa đủ thông tin”, so the other two messages must never be reachable from it — and its fix is always **Cập nhật hồ sơ**, never **Tải lên CV khác**. My CVs states this on the screen itself, because four chips in one list otherwise read as four chips any CV can carry.',
          ...
          },
          {
            ...
            table: {
              ...
              rows: [
                [
                  '**Not a CV**',
                  'Rose chip **✕ Bị từ chối — Không phải CV** · line: “File bạn tải lên không phải một CV.” → **Tải lên CV khác**',
                ...
                [
                  '**CV but not enough information**',
                  'Rose chip **✕ Bị từ chối — Chưa đủ thông tin** · line: “Hồ sơ chưa đủ thông tin để gửi tới nhà tuyển dụng.” → **Cập nhật hồ sơ**',
                ...
                [
                  '**Can’t read**',
                  'Rose chip **✕ Bị từ chối — Không đọc được** · line: “File dạng ảnh scan nên hệ thống không đọc được nội dung. Hãy tải lên bản PDF dạng văn bản.” → **Tải lên CV khác**',
                ...
                [
                  '— (no rejection yet: DOUBT, awaiting review — Can’t read · Not enough information)',
                  'Amber chip **⚠ Không đọc được · Chờ duyệt** / **⚠ Chưa đủ thông tin · Chờ duyệt** · line names the self-service exit too (fix the fields / replace the file — the scan re-runs, no admin needed)',
                  'Amber **Đang kiểm tra CV** · “Sẽ gửi tới nhà tuyển dụng ngay sau khi kiểm tra xong.” The application status underneath is **Not sent**',
                  '**Nothing** — the application has not been delivered, so no employer row exists to tag',
                ],
              ],
            },
            items: [
              'THE CHIP SAYS THE STATUS, THE LINE SAYS THE REASON — on all three surfaces. Per-reason chips would put three different red labels in one list for the reader to learn; the chip answers “what state is this row in”, the line under it answers “why, and what do I do now”.',
              'AMBER MEANS A WAIT, ROSE MEANS A VERDICT — the colour alone must say whether anything is still coming. The two doubt chips are the only amber ones, and they carry “Chờ duyệt” so the candidate can SEE the wait instead of assuming they are already findable. A rejected chip never says “tạm”.',
              ...
              'THE EMPLOYER NEVER SEES DOUBT — an application on a doubt CV was not delivered, so there is no employer row to tag. The two doubt tags exist only on the jobseeker side, which is why the doubt row’s employer cell is empty rather than a softer wording of “pending”.',
              'SARAMIN CVs APPEAR IN THIS TABLE ONCE, implicitly: their only failing state is “Chưa đủ thông tin” WITHOUT “Chờ duyệt” — nothing is queued for review, nothing was ever sent, so no application tag and no employer tag can exist for it.',
            ],
          },
          {
            early: true,
        ...
        behaviors: [
          ...
          'Applying with a “chưa đủ thông tin” CV always SUCCEEDS; the application then waits for the CV verdict. The candidate is told at that moment — “Đang kiểm tra CV — sẽ gửi sau khi duyệt” — never warned upfront, and never given a promised time we do not control.',
