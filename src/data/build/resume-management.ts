    ...
    },
    {
      ...
      table: {
        ...
        rows: [
          ...
          ['Expected salary', '7', 'The ranges overlap IN THE SAME CURRENCY — or the candidate’s salary is INTERVIEW / unset — or the two sides are in DIFFERENT currencies (scored neutral, see below).', '3 when the candidate’s minimum is within 20% above the job’s maximum, same currency.', 'Candidate’s minimum exceeds the job maximum by more than 20%, same currency.'],
  ...
  features: [
    ...
    //    Authored as a research/discovery brief because we have no data model yet.
    {
      ...
      detail: {
        ...
        keyPoints: [
          {
            vi: 'Một dòng kết quả phải đủ để quyết định có mở khoá hay không: giới tính · tuổi, số năm kinh nghiệm, công ty HIỆN TẠI đầy đủ (tên, chức danh, thời gian, tình trạng, mô tả 1 dòng), các công ty TRƯỚC ĐÓ dạng chip ngắn (tên · chức danh · tổng thời gian), phần còn lại gộp thành "+N", rồi học vấn và kỹ năng.',
            en: 'A result row must be enough to decide whether to unlock: gender · age, years of experience, the CURRENT job in full (company, title, period, employment status, one line of description), EARLIER jobs as compact chips (company · title · total duration), the rest collapsed into "+N", then education and skills.',
          },
          {
        ...
        uiFields: [
          ...
          },
          {
            ...
            items: [
              ...
              { name: 'current.period', type: 'derived string', required: true, notes: 'from–to, e.g. "03/2023 – nay". "nay" / "now" when isCurrent' },
              { name: 'current.employmentStatus', type: 'derived badge', required: true, notes: 'Đang làm việc (still employed) · Đã nghỉ (left) — derived from isCurrent on that entry, NOT a stored field. Toned green when still employed, because "currently employed" changes how a recruiter opens the conversation' },
              ...
              { name: 'prior[]', type: 'compact chips', notes: 'every earlier job as "Company · Job title (total duration)". NO description, NO from–to dates — the total duration is the signal (job-hopping vs. tenure). Masked to employer type while locked, like the current one' },
          ...
          {
            group: 'Filter rail · 3. CANDIDATE INFORMATION → Basic information (EXACTLY six)',
            items: [
              { name: 'the six', type: 'rule', required: true, notes: 'the client field sheet lists NINE Basic-information fields. Three are not filters: full name (masked until unlock) and email + phone (contact details, revealed by an unlock, never a search facet). The remaining SIX are the whole of this sub-group — nothing else may be added to it' },
              ...
              { name: 'gender', type: 'dropdown', notes: '⚠ the client’s own master data marks this "never a filter/matching key". Built on the 2026-08-12 instruction; needs written confirmation that the rule is being reversed' },
              { name: 'maritalStatus', type: 'dropdown', notes: '⚠ same 🔒 constraint, and the field with no defensible hiring purpose. See the ⚠ block' },
              { name: 'age (from dateOfBirth)', type: 'dropdown', notes: 'Under 25 · 25–34 · 35–44 · 45+ — BUCKETS, never a free min/max pair, so the UI cannot be pointed at a single birth year' },
        ...
        sections: [
          {
            ...
            table: {
              ...
              rows: [
                ...
                ['Current job — company, title, period, employment status', 'Shown, employer name masked to type + city', 'Employer named, plus the full work history'],
                ['Current job — description', 'Shown, clamped to ONE line', 'Shown in full'],
                ['Earlier jobs', 'Compact chips — company · title (total duration), employer masked. Overflow as "+N"', 'All of them, with dates and descriptions'],
          ...
          },
          {
            ...
            table: {
              ...
              rows: [
                ...
                ['Nationality', 'Real, and common in Vietnam: work-permit eligibility, native-speaker roles, foreign-invested employers with quotas.', 'NOT marked 🔒. A protected characteristic, but defensible where the requirement is genuine and recorded.'],
