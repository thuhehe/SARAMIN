        ...
        sections: [
          ...
          },
          {
            ...
            items: [
              ...
              'A CV NOBODY CHOSE HAS NO STATUS, it has an ABSENCE — printing “Not sent” or “Hidden” for a record that does not exist is how a list starts lying. So each cell now reads CHOICE then CONSEQUENCE: line 1 is what the CANDIDATE did (“2 đơn ứng tuyển” · “Đã chọn cho CV search” · “Chưa chọn”), line 2 is what our CV status does to that choice (“2 đơn đang giữ lại — chờ duyệt” · “Đang hiển thị với NTD” · “Bị ẩn — chờ duyệt”). When there is no choice there is deliberately NO second line.',
              'THE BLANK IS THE USEFUL PART. A queue row whose two cells both read “Chưa” is a row where the decision releases NOTHING and nobody is waiting — visible at a glance, and completely hidden by the old wording. It is also the fastest way to triage a backlog: rows with applications held are the ones costing someone something.',
              ...
              'THE APPLICANTS SCREEN KEEPS “Application status” UNCHANGED. A row there IS an application, so it genuinely has a status. The rename applies only where a row is a CV — CV review and Talent pool, which share one cell component so the two cannot drift.',
