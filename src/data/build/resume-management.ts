        ...
        sections: [
          {
            ...
            items: [
              ...
              'PHOTO AND DATE OF BIRTH ARE SHOWN AT TIER 2 (client decision, 2026-08-23), recorded with its cost so the trade is not rediscovered later as a surprise: a face is reverse-searchable and is how a manager recognises their own employee, and a full date of birth is half of a standard identity pair. Neither is closed by the tiering. The mitigation is the company blocklist below rather than more masking — photo and age are also what a recruiter reads when deciding whether to spend a credit at all, so hiding them would cost unlock conversion to close a leak the blocklist closes more precisely.',
              ...
              'OPEN — THE RESIDUAL RE-IDENTIFICATION RISK AND THE FIX WE RECOMMEND. Employer + title + age + city + photo is often enough to identify someone, and the party most motivated to try is the candidate’s CURRENT employer. Hiding company names would close it and destroy the product’s value, so that is the wrong lever. The standard answer is a candidate-side **company blocklist** — the jobseeker names companies that must never see their CV in search (Saramin KR has this). It protects the one case that matters without weakening the pool for every other employer. Needs a client decision; not in Phase-1 scope yet.',
            ],
          },
          {
            early: true,
