...
/* The candidate drill-in modal — Basic information · Work preference · CV content —
   lives in ui/candidateProfile so the Talent pool and the Jobseeker account screen
...
export function AdminResumes() {
  ...
  /* CV STATUS AS A REAL FILTER, not an inferred one. ListPage can usually derive
     a filter from a column whose values repeat — but not this column: the admin's
     recorded `why` renders inside the same cell, so the cell TEXT is unique per row
     and the derivation (which caps values at 24 chars) correctly rejects it.
     Deriving filters from rendered text is right for most lists and wrong here, so
     the screen supplies its own and filters on the FIELD. */
  const [stPick, setStPick] = useState('')
  const visible = stPick ? raw.filter((r) => stOf(r) === stPick) : raw
  const rows = visible.map((r, i) => [
    ...
    /* ONE status — and the sub-line is the REJECTION REASON only. Approval notes
       ("real CV, just a scan") used to print here too and are gone: an approved
       row's status explains itself, while a rejected row without its reason is a
       verdict nobody can review. The note survives in the record either way. */
    <div className="min-w-0">
      <Pill tone={ST_TONE[stOf(r)]}>{stOf(r)}</Pill>
      {stOf(r) === 'Rejected' && r.why && <p className="mt-0.5 truncate text-[10.5px] text-rose-600" title={r.why}>{r.why}</p>}
    </div>,
    /* Plain grey like every other data cell — the amber "thin CV" emphasis is
       gone: the status column already carries the judgement, and a second
       coloured signal on the same row competed with it. */
    /* Binary by construction: `picked` is always true in the pool, so this can
       only render Showing or Hidden — never the "chưa chọn" state that CV review
       needs. Same component; the screen's data decides which states are reachable. */
    <SearchCell picked verdict={stOf(r) === 'Qualified' ? 'qualified' : stOf(r) === 'Rejected' ? 'rejected' : 'doubt'} />,
    <span className="truncate text-muted">{r.content}</span>,
    stOf(r) === 'Qualified'
  ...
  return (
    <div>
      ...
      <p className="mb-2.5 rounded-lg border border-line bg-canvas/50 px-3 py-2 text-[11.5px] leading-relaxed text-muted">
        ...
        <b className="font-semibold text-ink/80">CV review</b> is the WORK QUEUE over the same records — finite, and meant to reach zero. This screen is the
        CATALOGUE: it never reaches zero, so it browses by whether an employer can find the person, and leaves the reviewing to the queue.{' '}
      ...
      <ListPage
        ...
        /* NO TABS — deliberately, since 2026-08-23. This row used to read
           ...
           them. That was the real duplication with CV review, and it is why the two
           screens reported different pending counts.

           THE RULE: on a QUEUE you work through tabs; on a CATALOGUE you filter. A
           tab is a promise that this subset is a JOB, and a catalogue — 8,420 rows
           that never reach zero — cannot keep that promise. CV review keeps its
           tabs because it IS the job.

           Nothing is lost by removing them: CV status is still one click away, as an
           explicit filter below. The difference is that a filter says "let me look",
           which is what this screen is for, while a tab said "here is work", which
           it never was.

           The badge on CV review in the nav is what replaces them — a queue nobody
           knows has work in it is the same as no queue at all. */
        filters={
          <FilterBar count={stPick ? 1 : 0} onClear={() => setStPick('')}>
            <FilterRow label="CV status" value={stPick} onChange={setStPick} options={['Qualified', 'Not enough information', "Can't read", 'Rejected']} />
          </FilterBar>
        }
        /* "Dùng để ứng tuyển" is the only column that does not belong here — this
           screen lists CVs, not applications. CV Search status stays: every row is
           a candidate who switched CV search ON, so the cell is BINARY here
           (Showing · Hidden) and answers the question the pool exists for — can an
           employer actually find this person right now. */
        cols={CV_COLS.filter((c) => c.label !== 'Dùng để ứng tuyển')}
        rows={rows}
      />
      {sel && <CandidateProfileModal name={sel} onClose={() => setSel(null)} />}
