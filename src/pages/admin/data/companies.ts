...
/* FOUR classifications. `ca-nhan` — “Cá nhân không có CCCD” — was briefly dropped
   on the argument that a B2B platform never sells to an unidentified buyer; it is
   back by client decision (2026-08-28). Điểm 4b, Phụ lục NĐ 254/2026 explicitly
   provides for it: a buyer who does not give tên + địa chỉ + số định danh cá nhân
   gets an invoice reading only “Bán cho người tiêu dùng”. The shape exists in the
   law we issue invoices under, so the system has to be able to produce it — and a
   rep who meets the case with no classification for it will pick a wrong one. */
...
export const COMPANIES: Company[] = [
  ...
  /* SELF-REGISTERED, PLACED BY AN ADMIN (09/2026 flow): the sign-up created only a
     row on Sign-ups; an admin resolved it with "Create company + place", which is
     what created this record and turned the login on. No sales owner yet, no
     registered address (the sign-up form has no address field), no documents. TWO
     of the three Verify inputs are missing (address · ERC), so this is the row the
     "Thiếu hồ sơ" filter finds and the Verify button stays disabled on. The
     employer is inside the console and can fix exactly that. */
  { name: 'Công ty CP NewCo', shortName: 'NewCo', legalName: 'Công ty Cổ phần NewCo', country: 'Việt Nam', tax: '0399xxxxxx', industry: 'CNTT', size: '10–49', address: '', contact: 'Ms. Phạm Thu Trang · HR', owner: NO_OWNER, status: 'Lost', account: 'New', lastPO: '—', renewal: '—', nextStep: 'Chờ hồ sơ: địa chỉ đăng ký MST + ERC', idle: null, note: 'Tự đăng ký 08/09, admin tạo công ty + place cùng ngày — chưa có địa chỉ đăng ký MST, chưa có ERC.', revenue: 0, jobPosting: false, resumeSearch: false, jobLeft: 0, jobTotal: 0, cvLeft: 0, cvTotal: 0, hasPage: false, jobs: 0, domain: 'newco.vn', since: '—',
    verification: { state: 'unverified', reason: 'new', since: '08/09/2026' } },
  /* PLACED, AND THE EMPLOYER FINISHED THE RECORD the next day — typed the registered
     address on Company information and uploaded the ERC (two pages). Nothing is
     missing, nobody has pressed Verify yet: this is the row the "Chờ xác minh"
     filter and the "Chờ verify" counter exist for. */
  { name: 'Công ty TNHH Giải pháp Số Việt', shortName: 'Số Việt', legalName: 'Công ty TNHH Giải pháp Số Việt', country: 'Việt Nam', tax: '0398xxxxxx', industry: 'CNTT', size: '10–49', address: 'Quận Cầu Giấy, Hà Nội', contact: 'Mr. Đỗ Quang Minh · Giám đốc', owner: NO_OWNER, status: 'Lost', account: 'New', lastPO: '—', renewal: '—', nextStep: 'Verify công ty — hồ sơ đã đủ', idle: null, note: 'Tự đăng ký 06/09, admin place ngay — employer đã điền địa chỉ đăng ký MST + ERC 2 trang; chờ admin Verify.', revenue: 0, jobPosting: false, resumeSearch: false, jobLeft: 0, jobTotal: 0, cvLeft: 0, cvTotal: 0, hasPage: false, jobs: 0, domain: 'sovietsolutions.vn', since: '—',
